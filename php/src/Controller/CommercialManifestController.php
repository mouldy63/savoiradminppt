...
class CommercialManifestController extends SecureAppController
{
	...
	public function index() {
		...
		if ($consigneeaddress=='') {
			...
			foreach ($query as $row) {
				if ($counter == 1) {
					$compareaddress1=$this->getInvoiceAddress($row['purchase_no']);
				}
				$compareaddress2=$this->getInvoiceAddress($row['purchase_no']);
...
// 			}
			...
			if ($legsinc=='y' && $exportData['legspackedwith'] =='') {
			    ...
			    $component7 .= "<td  class='".$borderstyle."' width=40 valign='top'>".$exportData['legsNW']."</td>";
			    ...
			    if ($wrapid == 4) {
			        ...
			        foreach ($exportData['acc'] as $accline) {
			            ...
			            if ($packinfo == null || $packinfo['PackedWith'] == null || $packinfo['PackedWith'] == 0) {
							...
							$accprodweight=0;
							if ($packinfo['ProductWgt'] != '' && $packinfo['ProductWgt'] != '0') {
								$accprodweight=intval($packinfo['ProductWgt']);
							}
							$accgrossweight=0;
							if ($packinfo['packkg'] != '' && $packinfo['packkg'] != '0') {
								$accgrossweight=intval($packinfo['packkg']);
							}
							$accdims='';
							if (!empty($packinfo['packwidth'])) {
								$accdims=intval($packinfo['packwidth']) . 'x';
							}
							if (!empty($packinfo['packheight'])) {
								$accdims.=intval($packinfo['packheight']) . 'x';
							}
							if (!empty($packinfo['packdepth'])) {
								$accdims.=intval($packinfo['packdepth'])."cm";
							}
			                $tmp = "<td class='".$borderstyle."' width=47 valign='top'>".$count." of ".$totalitemsinorder." ".$wrap['pdfwrapname']."</td>";
			                $tmp .= "<td  class='".$borderstyle."' width=130 valign='top'>".$accline['description']."</td>";
			                $tmp .= "<td  class='".$borderstyle."' width=40 valign='top'>".$accdims."</td>";
			                $tmp .= "<td  class='".$borderstyle."' width=40 valign='top'>".$accgrossweight."</td>";
			                $tmp .= "<td  class='".$borderstyle."' width=40 valign='top'>".$accprodweight."</td>";
			                array_push($component9, $tmp);
			                $count+=1;
			                $linetotal+=1;
			            }
	...
	public function getInvoiceAddress($pno) {
	    $query = $this->Purchase->find()->where(['PURCHASE_No' => $pno]);
	    $purchase = null;
			foreach ($query as $row) {
				$purchase = $row;
		...
		foreach ($query as $row) {
			...
			if (isset($address['street1'])) {
				$invoiceaddress.=$address['street1']."<br>";
			}
			if (isset($address['street2'])) {
				$invoiceaddress.=$address['street2']."<br>";
			}
			if (isset($address['street3'])) {
				$invoiceaddress.=$address['street3']."<br>";
			}
			if (isset($address['town'])) {
				$invoiceaddress.=$address['town']."<br>";
			}
			if (isset($address['county'])) {
				$invoiceaddress.=$address['county']."<br>";
			}
			if (isset($address['postcode'])) {
				$invoiceaddress.=$address['postcode']."<br>";
			}
