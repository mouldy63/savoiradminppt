...
class OutstandingCoBrochureRequestsController extends SecureAppController
{
    ...
    public function index() {
		...
		$brochurerequests = $contactTable->getOutstandingCoRequests($this->SavoirSecurity->isSuperuser(), $this->getCurrentUserRegionId(), $this->getCurrentUserLocationId());
	...
	public function print() {
		if (!$this->request->is('post')) {
			...
			$this->redirect(array('controller' => 'OutstandingCoBrochureRequests', 'action' => 'index'));
			return;
    	}
		...
		foreach ($corresQuery as $corresdata) {
			$corresid = $corresdata['correspondenceid'];
			$greeting = $corresdata['greeting'];
			$correspondence = $corresdata['correspondence'];
			} 
		} 
		if (count($corresQuery) > 0) {
	...
	public function export() {
    	...
    	$this->setResponse($this->getResponse()->withDownload('OutstandingCompanyBrochureRequests.csv'));
