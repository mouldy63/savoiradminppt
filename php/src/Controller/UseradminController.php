...
class UseradminController extends SecureAppController
{
    ...
    private $successMessages = [
        1 => 'The user %u has been created',
        2 => 'Roles successfully updated'
    ];
    private $errorMessages = [
        1 => 'Use must be a superuser to use this function',
        2 => 'No user id set',
        3 => 'Unknown error',
    ...
    public function index()
    {
        ...
        if (isset($query['errno']) && array_key_exists($query['errno'], $this->errorMessages)) {
            $this->Flash->error($this->errorMessages[$query['errno']]);
        }
    }
    ...
    public function userDetail()
    {
        ...
        $this->set('data', $userinfo);
        $this->viewBuilder()->setLayout('fabricstatus');
        if (isset($query['msgno']) && array_key_exists($query['msgno'], $this->successMessages)) {
            $msg = $this->successMessages[$query['msgno']];
            $msg = sprintf($msg, $id);
    ...
    private function _sendEmail($email, $choice, $info, $username)
    {
        ...
        $Email->setFrom(array('info@savoirbeds.co.uk' => 'Savoir Admin'));
        $Email->setTo($email);
        $Email->setCc('it-spt@savoirbeds.co.uk');
