...
public function getProdTopper($datefrom, $dateto, $madeat) {
	...
	public function getHBOBCSV($madeat) {
    	$sql = "Select P.purchase_no, P.headboardwidth as w, P.headboardheight as l, P.order_number, P.headboardstyle as n, P.specialinstructionsheadboard as x, P.headboardfabric, P.headboardfabricchoice, P.headboardfabricdesc, L.adminheading";
    	$sql .= " from qc_history_latest Q, purchase P, Location L WHERE (P.cancelled is Null or P.cancelled='n') and  P.purchase_no=Q.purchase_no and P.idlocation=L.idlocation and Q.componentID=8 and Q.madeat=". $madeat ." and P.code<>15919 and Q.finished is Null and (Q.QC_StatusID=0 or Q.QC_StatusID=2 or Q.QC_StatusID=4 or Q.QC_StatusID=20) and P.code<>15919 ";
    	$myconn = ConnectionManager::get('default');
		$data= $myconn->execute($sql)->fetchAll('assoc');
		return $data;
    ...
    public function getItemMadeAt($madeat) {
    	$sql = "Select ManufacturedAt from manufacturedat where ManufacturedAtID=". $madeat ."";
    	$myconn = ConnectionManager::get('default');
		return $myconn->execute($sql)->fetchAll('assoc')[0]['ManufacturedAt'];
    }
    
    public function getItemStatus($status) {
    	$sql = "Select QC_status from qc_status where QC_statusID=". $status ."";
    	$myconn = ConnectionManager::get('default');
		return $myconn->execute($sql)->fetchAll('assoc')[0]['QC_status'];
    }
