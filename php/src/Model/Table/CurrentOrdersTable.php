...
class CurrentOrdersTable extends Table {
	...
	public function getCurrentOrders($sortorder, $security, $showroom) {
		
		$sql = "Select * from (";
		$sql .= "select ORDER_NUMBER,ORDER_DATE,acknowdate,P.PURCHASE_No,surname,P.OWNING_REGION,P.customerreference,c.title,first,company,total,ordercurrency,paymentstotal,balanceoutstanding,vat,deliverypostcode,productiondate,bookeddeliverydate,overseasOrder,istrade,";
		if ($security->_userHasRole('ADMINISTRATOR') || $security->_userHasRole('SHOWROOM_VIEWER') || $security->getCurrentUserRegionId()==1) { 	
			$sql .= "adminheading,";
		}
		$sql .= " mcs.mincollectionstatus,";
		...
		$sql .= " from address A join contact C on A.CODE = C.CODE";
		$sql .= " join purchase P on C.CONTACT_NO = P.contact_no";
		if ($security->_userHasRole('ADMINISTRATOR') || $security->_userHasRole('SHOWROOM_VIEWER') || $security->getCurrentUserRegionId()==1) { 	
			$sql .= " join location L on P.idlocation = L.idlocation";
		}
		$sql .= " left join mincollectionstatus mcs on mcs.purchase_no=p.purchase_no";
		...
		$sql .= " and (mincollectionstatus is null or mincollectionstatus < 4)";
	...
	public function getCurrentOrdersSlow($sortorder, $security, $showroom) {
