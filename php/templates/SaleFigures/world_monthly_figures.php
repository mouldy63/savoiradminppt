...
<?php 
	...
	$monthlyDetail = $data['monthly_figure_details'];
	...
	echo $this->element('saleFiguresTest',array('test'=>$data['test'],'action'=>'month'));
?>
...
<tr style="background-color:white;">
	...
	<th class = "tablecell bordercell">Year to Date VS Prior Year to Date in Sales Currency</th>
	<th class = "tablecell bordercell">Year to Date VS Prior Year to Date in %</th>
	<th class = "tablecell bordercell">Year to Date Actual VS Year to Date Target in Sales Currency</th>
...
<?php
	...
	foreach($allFigures as $yearKey=>$tempYearFigures):
		...
		$interimTotalArray=array_fill(1,10,0);
		foreach($tempYearFigures as $showroomKey=>$showroomFigures):
			$showroomTargets = $tempYearTargets[$showroomKey];
?>
	<tr>
		<td class = "twotablecell bordercell">
			<?php echo $showroomFigures['showroomName']?>
			<?php if ($showroomKey != 99) { ?>
				(<span class="native_currency_code" data-currency-code="<?php echo $showroomFigures['currency'];?>"><?php echo $showroomFigures['currency'];?></span>)
				<button onclick="printMonthlyReport(<?php echo $showroomKey;?>)">Print</button>
			<?php } ?>
		</td>
<?php
            $isGBP = true;
			...
			if(!$isGBP){
                ...
                $interimTotalArray[1] += $showroomFigures['figures'][$currentMonth]/$exchangeRate[$yearKey][$currentMonth][$showroomFigures['currency']];
                $interimTotalArray[2] += $showroomTargets[$currentMonth]['amount']/$exchangeRate[$yearKey][$currentMonth][$showroomFigures['currency']];
                $interimTotalArray[3] += $allFiguresLastYear[$showroomKey]['figures'][$currentMonth]/$exchangeRate[$yearKey-1][$currentMonth][$showroomFigures['currency']];
                $interimTotalArray[4] += $lastyearYTDGBP;
                $interimTotalArray[5] += $thisyearTargetYTDGBP;
                $interimTotalArray[6] += $thisyearYTDGBP;
                $interimTotalArray[7] += ($thisyearYTDGBP - $lastyearYTDGBP);
                $interimTotalArray[9] += ($thisyearYTDGBP - $thisyearTargetYTDGBP);
			}
			else{
                ...
                $interimTotalArray[1] += $showroomFigures['figures'][$currentMonth];
                $interimTotalArray[2] += $showroomTargets[$currentMonth]['amount'];
                $interimTotalArray[3] += $allFiguresLastYear[$showroomKey]['figures'][$currentMonth];
                $interimTotalArray[4] += $lastyearYTD;
                $interimTotalArray[5] += $thisyearTargetYTD;
                $interimTotalArray[6] += $thisyearYTD;
                $interimTotalArray[7] += ($thisyearYTD - $lastyearYTD);
                $interimTotalArray[9] += ($thisyearYTD - $thisyearTargetYTD);
			}
?>
		...
		<td class = "tablecell bordercell"><?php echo $lastyearYTD==0?0:(round((($thisyearYTD-$lastyearYTD)/$lastyearYTD)*10000)/100).'%'; ?></td>
		<td class = "tablecell bordercell <?php echo $currencyStatus;?>" 
			<?php if(!$isGBP):?>
		...
		<td class = "tablecell bordercell"><?php echo $thisyearTargetYTD==0?0:(round((($thisyearYTD-$thisyearTargetYTD)/$thisyearTargetYTD)*10000)/100).'%'; ?></td>
	</tr>
<?php
		$totalArrayKey++;
		if($showroomKey == 99):
?>
        <tr>
        	<td class = "twotablecell bordercell">Owned Subtotal (GBP):</td>
        <?php
        	for($ii=1;$ii<=10;$ii++):
        		if($ii==8||$ii==10):
        ?>
        	<td class = "tablecell bordercell"></td>		
        <?php 	else:?>
        	<td class = "tablecell bordercell"><?php echo number_format($interimTotalArray[$ii],0,'.',',');?></td>
...
?>
<tr>
	<td class = "twotablecell bordercell">Dealer Subtotal (GBP):</td>
<?php
	for($ii=1;$ii<=10;$ii++):
		if($ii==8||$ii==10):
?>
...
allTargetsData = {
	...
	monthlyDetail:<?php echo json_encode($monthlyDetail);?>,
...
function worldTableGenerator(allRowFigures,allRowTargets){
	...
	str += '</tr>';
	var showroomSBCode = [1,48,49,50,3,4,36,27,24,17,37,34,39,99];
	...
	for(var yearKey in allFigures){
		...
		for(var showroomKey in tempYearFigures){
			...
			var tempStr='';
			tempStr += '<tr><td class = "twotablecell bordercell">' + showroomFigures['showroomName'];
			if (showroomKey != 99) {
				tempStr += ' (<span class="native_currency_code" data-currency-code="' + showroomFigures['currency'] + '">' + 
			showroomFigures['currency'] + '</span>)' + 
				'<button onclick="printMonthlyReport(' + showroomKey + ')">Print</button>';
			}
			tempStr += '</td>';
			isGBP = true;
            currencyStatus = '';
