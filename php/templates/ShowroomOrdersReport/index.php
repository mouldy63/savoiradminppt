...
<?php $slct = (in_array('n', $showroom)) ? "selected" : "";   ?>
<option <?= $slct ?> value="n">Current Showrooms:  </option>
<?php foreach ($activeshowrooms as $row): 
//$cheked='';
//			if (in_array($row['idlocation'], $showroom)) {
//			$cheked='selected';
//			};
?> 
<?php $slct = (in_array($row['idlocation'], $showroom)) ? "selected" : "";   ?>              
...
<?php $slct = (in_array('n', $ordersource)) ? "selected" : "";   ?>
<option <?= $slct ?> value="n">Order Source:  </option>
<option <?= $slct ?> value="n">All</option>
<?php foreach ($ordersources as $row): ?> 
<?php $slct = (in_array($row['orderSource'], $ordersource)) ? "selected" : "";   ?> 
...
<tr><td><b>TICK TO EXCLUDE:</b></td>
<td><input type="checkbox" id="cancelled" name="cancelled" value="y" <?=empty($cancelledorders)?'':'checked' ?> > Cancelled Orders</td>
<td><input type="checkbox" id="onhold" name="onhold" value="y" <?=empty($onhold)?'':'checked' ?> > Orders On Hold</td>
<td><input type="checkbox" id="quotes" name="quotes" value="y" <?=empty($quotes)?'':'checked' ?> > Quotes</td>
<td></td></tr>
<tr><td colspan="4">
...
<?php 
$showroomlist='';
foreach ($data['showroom_list'] as $showroom):
$showroomlist .= $showroom['idlocation'].",";
//debug($showroomlist);
 endforeach; 
;?>
	<?php
				...
				<tr class="tablebd">
					...
					<?php foreach ($data['showroom_list'] as $showroom): ?> 
						<?php if (isset($item['data'][$showroom['idlocation']])) { 
						 ?>
							<?php $link = "ShowroomOrdersReport/orderReport?idlocation=".$showroom['idlocation']."&ordersource=".implode(',',$ordersource)."&key=".$item['key']."&monthfrom=".urlencode($monthfrom)."&monthto=".urlencode($monthto); ?>
					...
					<?php if ($item['data'][0] > 0) { ?>
						<?php $link = "ShowroomOrdersReport/orderReport?idlocation=".$showroomlist."&ordersource=".implode(',',$ordersource)."&key=".$item['key']."&monthfrom=".urlencode($monthfrom)."&monthto=".urlencode($monthto); ?>
