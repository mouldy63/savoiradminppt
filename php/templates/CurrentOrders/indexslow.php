...
if ($row['overseasOrder']=='y') {
	$lorrycount=0;
	$sql="select count(*) as lorrycount from (SELECT exportcollectionid FROM exportlinks E, exportcollshowrooms L where E.linkscollectionid=L.exportCollshowroomsID and purchase_no=".$row['PURCHASE_No']." group by exportcollectionid)  as x";
	...
	if ($lorrycount>1) {
       ...
       $sql2="Select * from exportcollections E, exportLinks L, exportCollShowrooms S where L.purchase_no=".$row['PURCHASE_No']." and L.linksCollectionID=S.exportCollshowroomsID and S.exportCollectionID=E.exportCollectionsID";
       $rs2 = $this->AuxiliaryData->getDataArray($sql2,[]);
       if (count($rs2)>0) {
         echo date("d/m/Y", strtotime(substr($rs2[0]['CollectionDate'],0,10)));
       } else {
         echo 'TBA';
       }
    }
