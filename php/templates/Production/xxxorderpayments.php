...
<div class="row">
    ...
    <?php if ($purchase['quote']=='n') { 
        if (!empty($orderhasexportsnoinvoices) != '' || !empty($orderhasexports) != '') { ?>
        ...
        <?php } ?>
            <?php if (!empty($orderhasexports)) { ?>
                ...
                foreach ($orderhasexports as $exportrow):
                    ...
                    $expdateUK=date("d/m/Y", strtotime(substr($exportrow['CollectionDate'],0,10)));
                ...
                endforeach;?>
                        ...
                        <td valign="top" align="left">
                            <input type="radio" name="exportchoice" id="exportchoice" value="<?=$expdateUK?>" onChange="pushInvoiceInfo('<?=$rows['invoiceNo']?>','<?=$invdate ?>','<?=$rows['exportCollectionsID']?>')">&nbsp;<?=$invdate ?>
                            <input type="hidden" name="cid" id="cid" value="<?=$cid?>" /></td>
                        <td valign="top" align="right"><?=$invno?></td>
                        
                        <td valign="top" align="right"><input type="hidden" name="T<?=$invno?>" id="T<?=$invno?>" value="<?=$totalcompprice?>" /><?=$this->OrderForm->getCurrencySymbol()?><?=number_format($totalcompprice,2)?></td>
                ...
                foreach ($orderhasexportsnoinvoices as $exportNIrow):
                    ...
                    foreach ($rs as $rows):
                        ...
                        <input type="radio" name="exportchoice" id="exportchoice" value="<?=$expdateUK?>" onChange="clearInvoiceInfo()">&nbsp;<strong>Ex&nbsp;works&nbsp;Date:</strong>&nbsp;<?=$exportNIrow['CollectionDate']?>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<strong>Amount:</strong>&nbsp;<?=$this->OrderForm->getCurrencySymbol()?><?=number_format($totalcompprice2,2)?><br>
                        <?php 
