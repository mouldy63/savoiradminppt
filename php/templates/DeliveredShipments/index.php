...
<?php foreach ($deliveredExports as $row): ?>
	 ...
	 <td><a href="/php/containerdetails?id=<?=$row["exportCollectionsID"]?>"><?php echo date("d/m/Y", strtotime(substr($row['CollectionDate'],0,10))) ?></a></td>
	...
	$nrows=0;
	foreach ($rs as $rows):
		echo "<a href='/php/shipmentdetails?location=". $rows["x"] ."&id=" .$row["exportCollectionsID"] ."'>". $rows['n'] ." (". $rows['y'] .")</a><br />";
		$nrows=$nrows+1;
	endforeach; 
	if ($nrows==0) {
	 echo "0";
