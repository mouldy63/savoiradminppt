$(document).ready(function() {
	...
	subcomponentLoadWatch('base_upholsteryreq', 'n', 17, 380);
	...
	$("#base_upholsteryreq").change(function() {
		setFabricReqHiddenField();
	});

	showHideLinkFinishSelect();
	$("#base_linkposition").change(function() {
		showHideLinkFinishSelect();
	});
});
...
function setFabricReqHiddenField() {
	var baseFabricReq = 'y';
	if ($("#base_upholsteryreq").val() == 'n') {
		baseFabricReq = 'n';
	}
	$("#base_fabricreq").val(baseFabricReq);
}

function showHideLinkFinishSelect() {
    var linkPosition = $("#base_linkposition").val();

    if (linkPosition && (linkPosition == 'No Link Required')) {
