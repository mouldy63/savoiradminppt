...
orderno=rs("order_number")
					...
					Set rs2 = getMysqlQueryRecordSet("Select * from componentdata where componentid=1 and componentname = '" & rs("savoirmodel") & "'", con)
						...
						if rs("componentid")="6" then 
							...
							valanceprice=0
							if rs("valfabricprice")<>"" and NOT isNull(rs("valfabricprice")) then valanceprice=valanceprice+CDbl(rs("valfabricprice"))
							if rs("valanceprice")<>"" then valanceprice=valanceprice+CDbl(rs("valanceprice"))
							componentpricearray(count)=valanceprice
							totalcost=totalcost+CDbl(componentpricearray(count))
						end if
						...
						Set rs2 = getMysqlQueryRecordSet("Select * from componentdata where componentid=8 and componentname = '" & rs("headboardstyle") & "'", con)
							...
							headboardprice=0
							if rs("hbfabricprice")<>"" and NOT isNull(rs("hbfabricprice")) then headboardprice=headboardprice+CDbl(rs("hbfabricprice"))
							if rs("headboardprice")<>"" and NOT isNull(rs("headboardprice")) then headboardprice=headboardprice+CDbl(rs("headboardprice"))
							componentpricearray(count)=headboardprice
							totalcost=totalcost+CDbl(componentpricearray(count))
						end if
...
commercialordertable=commercialordertable & "<table width=""100%"" border=""1"" cellspacing=""2"" cellpadding=""1""><tr><td colspan=""3"">Number&nbsp;&&nbsp;Kind&nbsp;of&nbsp;Packages</td><td>Dimensions</td><td>Harmonized Tariff Code</td><td align=""right"">Weight<br />(kg)</td> <td>Qty</td>"
if NOT userHasRoleInList("NOPRICESUSER") then
	'if (retrieveuserid()<>181 and retrieveuserid()<>182) then) then
	commercialordertable=commercialordertable & "<td align=""right"">Unit&nbsp;Price</td><td align=""right"">Amount</td>"
end if
commercialordertable=commercialordertable & "</tr>"
for i= 1 to items
commercialordertable=commercialordertable & "<tr><td width=""70""> " & i & " of " & items & "</td><td width=""50"">" & wraptext2 & "&nbsp;</td><td width=""170"">" & componentarray(i) & "</td><td>" & dimensionsarray(i) & "</td><td>" & tarrifarray(i) & "</td><td align=""right"">" & weightarray(i) & "</td><td align=""center"">1</td>"
if NOT userHasRoleInList("NOPRICESUSER") then
	'if (retrieveuserid()<>181 and retrieveuserid()<>182) then) then
	commercialordertable=commercialordertable & "<td align=""right"">" & getCurrencySymbolForCurrency(orderCurrency)  & formatnumber(componentpricearray(i),2) & "</td><td align=""right"">" & getCurrencySymbolForCurrency(orderCurrency)  & formatnumber(componentpricearray(i),2) & "</td>"
end if
commercialordertable=commercialordertable & "</tr>"
next
commercialordertable=commercialordertable & "<tr><td width=""70""></td><td width=""50""></td><td width=""170""></td><td></td><td>VALUE</td><td align=""right""></td><td align=""center""></td><td align=""right""></td>"
if NOT userHasRoleInList("NOPRICESUSER") then
	'if (retrieveuserid()<>181 and retrieveuserid()<>182) then) then
	commercialordertable=commercialordertable & "<td align=""right"">" & getCurrencySymbolForCurrency(orderCurrency)  & formatnumber(totalcost,2) & "</td>"
end if
commercialordertable=commercialordertable & "</tr>"
...
if NOT userHasRoleInList("NOPRICESUSER") then
	...
	PDF.AddHTMLPos 130, 660, "<table width=""100%""><tr><td width=""300"" ><br><img src=""images/CI-sig.gif"" width=""349"" height=""118""></td><td width=""200"" align=""right""><br><br><br><br></td><td align=""right""><br><br><br></td></tr></table><br><table width=""100%""><tr><td align=""center"">VAT Reg No. GB 706 8175 27<br>Savoir Beds Limited, registered in England: No. 3395749.<br>Registered Address: 1 Old Oak Lane, London NW10 6UD, UK</td></tr></table>"
end if
