...
s = s & " <td width=""54"" valign=""top"">" & clienthdg & "</td><td width=""112"" valign=""top"">" & clientdetails & "</td><td width=""24""></td><td width=""166"">" & custaddress & "</td><td width=""24""></td><td width=""166"">" & deliveryaddress & "</td> "
s = s & " </tr> "
s = s & " </table> "
If quote="y" then 
...
If rs("legprice")<>"" and NOT ISNULL(rs("legprice")) then
	...
	if rs("baseprice")<>"" and NOT ISNULL(rs("legprice")) then 
...
if rs("baserequired")="y" then
	If rs("upholsteryprice")<>"" and NOT ISNULL(rs("upholsteryprice")) then
		If CDbl(rs("upholsteryprice"))>0.0 then basevalanceprice=basevalanceprice + CDbl(rs("upholsteryprice"))
	end if
...
If rs("topperrequired")="y" then
sumtbl=sumtbl & "<tr><td width=10></td><td width=215>Topper</td><td width=25 align=""right"">" & fmtCurr2(rs("topperprice"), true, rs("ordercurrency")) & "</td></tr>"	
end if
If (rs("valancerequired")="y" or upholsterysum<>0) then
'valance fabric & valance price added together
sumtbl=sumtbl & "<tr><td width=10></td><td width=215>Upholstery</td><td width=25 align=""right"">" & fmtCurr2(upholsterysum, true, rs("ordercurrency")) & "</td></tr>"	
end if
If rs("headboardrequired")="y" then
sumtbl=sumtbl & "<tr><td width=10></td><td width=215>Headboard</td><td width=25 align=""right"">" & fmtCurr2(rs("headboardprice"), true, rs("ordercurrency")) & "</td></tr>"	
end if
...
If rs("bookeddeliverydate")<>"" then
PDF.AddTextPos 490, 648, rs("deliverydate")
else
	if day(rs("deliverydate"))=15 then deltxt="Approx middle of " & monthname(month(rs("deliverydate"))) & " " & year(rs("deliverydate"))
	if day(rs("deliverydate"))=5 then deltxt="Approx beginning of " & monthname(month(rs("deliverydate"))) & " " & year(rs("deliverydate"))
	if day(rs("deliverydate"))=25 then deltxt="Approx end of " & monthname(month(rs("deliverydate"))) & " " & year(rs("deliverydate"))
PDF.AddTextPos 450, 648, deltxt
end if
