...
function commaWithoutDecimal($digit){
    //var p = s.indexOf('.');
    $x = explode('.',(string)$digit);
    $x1 = $x[0];
    $x2 = strlen($x[1]) >= 1 ? (strlen($x[1]) > 1?'.'.$x[1]:'.'.$x[1].'0'):"";
    $rgx = '/(\d+)(\d{3})/';
    while (preg_match($rgx,$x1)) {
        $x1 = preg_replace($rgx,'$1,$2',$x1);
    }
...
foreach($tableColumns as $item){
    ...
    foreach($showrooms["owned"] as $showroom){
    	...
    	$showroomCurrency = $_POST['showroomCurrency_'.$showroom];
        $actual = addComma($_POST['monthActual_'.$showroom]);
        $monthTarget = commaWithoutDecimal($_POST['monthTarget_'.$showroom]);
        $priorYear = addComma($_POST['priorYear_'.$showroom]);
        $priorYearToDate = addComma($_POST['priorYearToDate_'.$showroom]);
        $yearToDateTarget = commaWithoutDecimal($_POST['yearToDateTarget_'.$showroom]);
        ...
        $YTDVsPYTDSalesinPercent = commaWithoutDecimal($_POST['YTDVsPYTDSalesinPercent_'.$showroom]);
        ...
        //TOTALS
        $actualTotal = addComma($_POST['columnTotal_1' ]);
        $monthTargetTotal = addComma($_POST['columnTotal_2']);
        $priorYearTotal = addComma($_POST['columnTotal_3']);
        $priorYearToDateTotal = addComma($_POST['columnTotal_4']);
        $yearToDateTargetTotal = addComma($_POST['columnTotal_5']);
        $yearToDateActualTotal = addComma($_POST['columnTotal_6']);
        $YTDVsPYTDSalesCurrencyTotal = addComma($_POST['columnTotal_7']);
        $YTDVsYTDTargetinSalesCurrencyTotal = addComma($_POST['columnTotal_9']);

        $pdf->SetFont('Arial','B','5');

        if($showroomname =="Düsseldorf"){

            $pdf->Cell(30,( $cellHeight), iconv("UTF-8", "Windows-1252//TRANSLIT", "Düsseldorf"). " (".$showroomCurrency.")",1,0,'R');
        }
        else{
            $pdf->Cell(30,( $cellHeight), $showroomname. " (".$showroomCurrency.")",1,0,'R');
        }
    ...
    $pdf->Cell($cellWidth,( $cellHeight), $actualTotal,1,0,'R');
    $pdf->Cell($cellWidth,( $cellHeight), $monthTargetTotal,1,0,'R');
    $pdf->Cell($cellWidth,( $cellHeight), $priorYearTotal,1,0,'R');
    $pdf->Cell($cellWidth,( $cellHeight), $priorYearToDateTotal,1,0,'R');
    $pdf->Cell($cellWidth,( $cellHeight), $yearToDateTargetTotal,1,0,'R');
    $pdf->Cell($cellWidth,( $cellHeight), $yearToDateActualTotal,1,0,'R');
    $pdf->Cell($cellWidth,( $cellHeight), $YTDVsPYTDSalesCurrencyTotal,1,0,'R');
    $pdf->Cell($cellWidth,( $cellHeight), "",1,0,'R');
    $pdf->Cell($cellWidth,( $cellHeight), $YTDVsYTDTargetinSalesCurrencyTotal,1,0,'R');
    $pdf->Cell($cellWidth,( $cellHeight), "",1,1,'R');
