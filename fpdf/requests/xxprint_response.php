...
class PDF extends FPDF
{
    ...
    function Header() {
        if ($this->renderHeader) {
            $currencySymbol = $this->getCurrencySymbol($currencyShortCode);
...
//		die();
     ...
     * @return array
     */
    public function getOrderDetailHeader($currencyShortCode)
    {
        $currencySymbol = $this->getCurrencySymbol($currencyShortCode);
        $w_symbol = $this->GetStringWidth($currencySymbol);
        $w_space = 190;
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
	...
	if(!empty($data["order_detail"])){
		...
		if (!empty($customerOrders)) {
			$pdf->orderDetail($customerOrders, $data['currency'], 'Client', $pdf->addComma($data['source_totals']['Client']));
		}

		if (!empty($stockOrders)) {
			$pdf->orderDetail($stockOrders, $data['currency'], 'Stock', $pdf->addComma($data['source_totals']['Stock']));
		}

		if (!empty($floorstockOrders)) {
			$pdf->orderDetail($floorstockOrders, $data['currency'], 'Floor Stock', $pdf->addComma($data['source_totals']['Floorstock']));
		}

        if (!empty($marketingOrders)) {
            $pdf->orderDetail($marketingOrders, $data['currency'], 'Marketing', $pdf->addComma($data['source_totals']['Marketing'] ));

        }

		if (!empty($testOrders)) {
			$pdf->orderDetail($testOrders, $data['currency'], 'Test',$data['source_totals']['Test']);
		}

		if (!empty($ecomOrders)) {
			$pdf->orderDetail($ecomOrders, $data['currency'], 'Ecom', $pdf->addComma($data['source_totals']['Ecom'] ));
		}
		
		if (!empty($otherOrders)) {
			$pdf->orderDetail($otherOrders, $data['currency'], 'Other', $pdf->addComma($data['source_totals']['Other'] ));
		}
