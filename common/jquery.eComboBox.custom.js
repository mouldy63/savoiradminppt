...
(function( $ ){
  ...
  var methods = {
     init : function( options ) {
		...
		return this.each(function() { 
			...
			if (settings.allowNewElements){
				selectElement.append('<option>{New Entry}</option>');
			}
			...
			selectElement.keydown( function(e){
				...
				if( selected=="{New Entry}" || settings['editableElements'] ) {
	 ...
	 disableAddingNewElements: function(){
		...
		if(fChild.text() == "{NEW E{New Entry}"){
			fChild.remove();
		}
	 },
	 enableAddingNewElements: function(){
		var fChild = $(this).children().first();
		if(fChild.text() != "{New Entry}"){
			$(this).prepend('<option>{New Entry}</option>');
		}
	 },
