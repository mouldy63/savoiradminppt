...
function getMysqlQueryRecordSet(aSql, aCon)
	'call log(scriptname, "getMysqlQueryRecordSet: sql = " & aSql)
	...
	if err.number <> 0 then
		...
		response.write("<br>sql=" & aSql)
		response.end
	end if
	on error goto 0
end function

function getMysqlQueryRecordSetWithErrorLogging(aSql, aCon)
	'call log(scriptname, "getMysqlQueryRecordSetWithErrorLogging: sql = " & aSql)
	Set getMysqlQueryRecordSetWithErrorLogging = Server.CreateObject("ADODB.recordset")
	getMysqlQueryRecordSetWithErrorLogging.ActiveConnection = aCon
	getMysqlQueryRecordSetWithErrorLogging.CursorLocation = adUseClient
	getMysqlQueryRecordSetWithErrorLogging.CursorType = adOpenStatic
	getMysqlQueryRecordSetWithErrorLogging.LockType = adLockReadOnly
	on error resume next
		getMysqlQueryRecordSetWithErrorLogging.Open aSql
	if err.number <> 0 then
		response.write("<br>Error=" & err.description)
		'call log(scriptname, "Error sql = " & aSql)
		response.end
	end if
	on error goto 0
end function
...
function getMysqlUpdateRecordSet(aSql, aCon)
	'call log(scriptname, "getMysqlUpdateRecordSet: sql = " & aSql)
...
function getMysqlUpdateRecordSetWithErrorLogging(aSql, aCon)
	'call log(scriptname, "getMysqlUpdateRecordSetWithErrorLogging: sql = " & aSql)
	Set getMysqlUpdateRecordSetWithErrorLogging = Server.CreateObject("ADODB.recordset")
	getMysqlUpdateRecordSetWithErrorLogging.ActiveConnection = aCon
	getMysqlUpdateRecordSetWithErrorLogging.CursorLocation = adUseClient
	getMysqlUpdateRecordSetWithErrorLogging.CursorType = adOpenKeyset	'was adOpenDynamic
	getMysqlUpdateRecordSetWithErrorLogging.LockType = adLockOptimistic
	on error resume next
		getMysqlUpdateRecordSetWithErrorLogging.Open aSql
	if err.number <> 0 then
		response.write("<br>Error=" & err.description)
		'call log(scriptname, "Error sql = " & aSql)
		response.end
...
function defaultSplitString(byref astr, byref adefault)
	if isnull(astr) or astr = "" then
		defaultSplitString = adefault
	else
		defaultSplitString = split(astr, ",")(0)
	end if
end function
