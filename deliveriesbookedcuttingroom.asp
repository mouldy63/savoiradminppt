...
dim diff, factories, datenow, twoweeksdate, custname, madeat, hasLondonItems, compmadeat1, compmadeat3, compmadeat5, compmadeat8, compmadeat7, hasCardiffItems, dofweek,startweek,newweek, pnewweek, weekno, pweekno, weeknoadd, pweeknoadd, weekstarted, pweekstarted, data, pweekno2, weekno2, finishedComp1, finishedComp3, finishedComp5, finishedComp8, finishedComp7, ItemNotFinished, strdate, enddate, ItemsFinished, cuttingroomNoDays, realproductiondate, weekNoToDisplay, realProdStartDate, realProdEndDate
...
sql="Select cuttingroomnoitems from manufacturedat where ManufacturedAtId=" & madeat
Set rs = getMysqlQueryRecordSet(sql, con)
cuttingroomNoDays=rs("CuttingRoomNoItems")
rs.close
set rs=nothing
...
<tr><td height="35" colspan="10" bgcolor="#999999">&nbsp;<br>  <strong>Production Date</strong><div align="center" style="margin:0; padding:0; float:right; width:90%; font-size:14px;">CUTTING ROOM</div><br>&nbsp;</td></tr>
<tr><td colspan="10"><hr></td></tr>

<%if madeat=2 then
sql = "select * from address A, contact C, Purchase P, Location L Where P.code=A.code AND C.contact_no=P.contact_no AND P.completedorders='n' AND P.quote='n' AND P.completedorders='n' and P.orderonhold<>'y' and (P.cancelled is Null or P.cancelled='n') AND P.productiondate<>'' AND P.londonproductiondate<>'' and (P.DeliveryDateConfirmed<>'y' or P.DeliveryDateConfirmed is Null) AND P.source_site='SB' and P.idlocation=L.idlocation order by P.londonproductiondate asc "
end if 
if madeat=1 then
sql = "select * from address A, contact C, Purchase P, Location L Where P.code=A.code AND C.contact_no=P.contact_no AND P.completedorders='n' AND P.quote='n' AND P.completedorders='n' and P.orderonhold<>'y' and (P.cancelled is Null or P.cancelled='n') AND P.productiondate<>'' AND P.cardiffproductiondate<>'' and (P.DeliveryDateConfirmed<>'y' or P.DeliveryDateConfirmed is Null) AND P.source_site='SB' and P.idlocation=L.idlocation order by P.cardiffproductiondate asc "
end if 
...
finishedcomp1=isCompMachined(con, rs("purchase_no"), 1)
compmadeat3=getComponentCurrentMadeAt(con, rs("purchase_no"), 3)
finishedcomp3=isCompMachined(con, rs("purchase_no"), 3)
compmadeat5=getComponentCurrentMadeAt(con, rs("purchase_no"), 5)
finishedcomp5=isCompMachined(con, rs("purchase_no"), 5)
...
if (compmadeat1=1 and finishedcomp1="n") or (compmadeat3=1 and finishedcomp3="n") or (compmadeat5=1 and finishedcomp5="n") or (compmadeat8=1 and finishedcomp8="n") then
	hasCardiffItems = true
end if
if (compmadeat1=2 and finishedcomp1="n") or (compmadeat3=2 and finishedcomp3="n") or (compmadeat5=2 and finishedcomp5="n") or (compmadeat8=2 and finishedcomp8="n") then
	hasLondonItems = true
end if
ItemNotFinished=False
if finishedcomp1="n" or finishedcomp3="n" or finishedcomp5="n" or finishedcomp8="n" then
...
productiondate=(DateAdd("d",-cuttingroomNoDays,productiondate))
...
response.Write("<tr><td height=""17"" colspan=""2"" bgcolor=""#999999"">Week No. " & weekNoToDisplay & "</td><td colspan=""6"" bgcolor=""#999999"" align=""center"">" & getcomponentCuttingNoTobefinished(con, cuttingroomNoDays, realProdStartDate, realProdEndDate, madeat) & " items to finish</td><td height=""17"" colspan=""2"" bgcolor=""#999999"">Production&nbsp;Date </td></tr>")
...
%>
    ...
    <tr>
      ...
      <%if rs("savoirmodel")<>"" and NOT ISNULL(rs("savoirmodel")) and compmadeat1=madeat and finishedcomp1="n" then
	...
	%>
      ...
      <%end if%>
      <%if rs("basesavoirmodel")<>"" and NOT ISNULL(rs("basesavoirmodel")) and compmadeat3=madeat  and finishedcomp3="n"  then
	    %>
     ...
     <span style="white-space: pre;color:<%=getColourForEntry(con, rs("purchase_no"), "Cut", 3)%>">&#9608;</span>
      ...
      <%if rs("toppertype")<>"" and NOT ISNULL(rs("toppertype")) and compmadeat5=madeat and finishedcomp5="n"   then
	 %>
      ...
      <td width="1%" valign="top">&nbsp;</td>
      <%end if%>
      <%if rs("headboardstyle")<>"" and NOT ISNULL(rs("headboardstyle")) and compmadeat8=madeat and finishedcomp8="n"   then
	  %>
