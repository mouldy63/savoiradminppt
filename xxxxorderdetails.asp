...
dim leftTension
dim rightTension, z, sid, loc, cid, matt1cratewgt, matt2cratewgt, base1cratewgt, base2cratewgt, topper1cratewgt, hb1cratewgt,  hb2cratewgt, legs1cratewgt, legs2cratewgt, acc1cratewgt, acc2cratewgt, valance1cratewgt, valance2cratewgt, boxprodwgt6_1, crateprodwgt6_1, boxprodwgt7_1, crateprodwgt7_1
dim isVIP
leftTension = -1
rightTension = -1
legsPackedWithCheck=""
...
if rs("mattressrequired")="y" and rs("wrappingid")=4 then
	...
	If not rs2.eof then
		...
		matt1cratewgt=rs2("NetPackagingWgt")
		crateprodwgt1_1=rs2("ProductWgt")
		end if
	rs2.close
	set rs2=nothing
end if
...
if rs("mattressrequired")="y" and rs("wrappingid")=4 then
	...
	If not rs2.eof then
		...
		matt2cratewgt=rs2("NetPackagingWgt")
		crateprodwgt1_2=rs2("ProductWgt")
	end if
	rs2.close
	set rs2=nothing
end if
...
base1cratewgt=rs2("NetPackagingWgt")
crateprodwgt3_1=rs2("ProductWgt")
end if
rs2.close
set rs2=nothing
end if
...
if rs("baserequired")="y" and rs("wrappingid")=4 then
	...
	If not rs2.eof then
		...
		base2cratewgt=rs2("NetPackagingWgt")
		crateprodwgt3_2=rs2("ProductWgt")
...
topper1cratewgt=rs2("NetPackagingWgt")
crateprodwgt5_1=rs2("ProductWgt")
end if
rs2.close
set rs2=nothing
end if
...
If not rs2.eof then
	...
	if not IsNull(rs2("packkg")) and Len(Trim(rs2("packkg"))) > 0 then legs1_packkg = FormatNumber(rs2("packkg"),0,,,0)
	if not IsNull(rs2("ProductWgt")) and Len(Trim(rs2("ProductWgt"))) > 0 then crateprodwgt7_1 = FormatNumber(rs2("ProductWgt"),0,,,0)
	if not IsNull(rs2("NetPackagingWgt")) and Len(Trim(rs2("NetPackagingWgt"))) > 0 then legs1cratewgt=FormatNumber(rs2("NetPackagingWgt"),0,,,0)
...
if not IsNull(rs2("packkg")) and Len(Trim(rs2("packkg"))) > 0 then valance1_packkg = FormatNumber(rs2("packkg"),0,,,0)
if not IsNull(rs2("ProductWgt")) and Len(Trim(rs2("ProductWgt"))) > 0 then crateprodwgt6_1 = FormatNumber(rs2("ProductWgt"),0,,,0)
if not IsNull(rs2("NetPackagingWgt")) and Len(Trim(rs2("NetPackagingWgt"))) > 0 then valance1cratewgt = FormatNumber(rs2("NetPackagingWgt"),0,,,0)
...
if not IsNull(rs2("packkg")) and Len(Trim(rs2("packkg"))) > 0 then acc1_packkg = FormatNumber(rs2("packkg"),0,,,0)
if not IsNull(rs2("ProductWgt")) and Len(Trim(rs2("ProductWgt"))) > 0 then crateprodwgt9_1 = FormatNumber(rs2("ProductWgt"),0,,,0)
if not IsNull(rs2("NetPackagingWgt")) and Len(Trim(rs2("NetPackagingWgt"))) > 0 then acc1cratewgt = FormatNumber(rs2("NetPackagingWgt"),0,,,0)
...
sql = "Select * from packagingdata where componentid=9 and comppartno=2 AND purchase_no = " & rs("purchase_no")
	...
	If not rs2.eof then
		...
		if not IsNull(rs2("packkg")) and Len(Trim(rs2("packkg"))) > 0 then acc2_packkg = FormatNumber(rs2("packkg"),0,,,0)
if not IsNull(rs2("ProductWgt")) and Len(Trim(rs2("ProductWgt"))) > 0 then crateprodwgt9_2 = FormatNumber(rs2("ProductWgt"),0,,,0)
if not IsNull(rs2("NetPackagingWgt")) and Len(Trim(rs2("NetPackagingWgt"))) > 0 then acc2cratewgt = FormatNumber(rs2("NetPackagingWgt"),0,,,0)
...
if rs("headboardrequired")="y" and rs("wrappingid")=4 then
	...
	If not rs2.eof then
		...
		hb1cratewgt=rs2("NetPackagingWgt")
		crateprodwgt8_1=rs2("ProductWgt")
	end if
	rs2.close
	set rs2=nothing
end if
...
if rs("headboardrequired")="y" and rs("wrappingid")=4 then
	...
	If not rs2.eof then
		...
		hb2cratewgt=rs2("NetPackagingWgt")
		crateprodwgt8_2=rs2("ProductWgt")
	end if
	rs2.close
	set rs2=nothing
end if
...
if rs("mattressrequired")="y" and rs("wrappingid")=3 then
	...
	If not rs2.eof then
		...
		if not IsNull(rs2("packkg")) and Len(Trim(rs2("packkg"))) > 0 then matt1kg = FormatNumber(rs2("packkg"),0,,,0)
		if not IsNull(rs2("ProductWgt")) and Len(Trim(rs2("ProductWgt"))) > 0 then boxprodwgt1_1 = FormatNumber(rs2("ProductWgt"),0,,,0)
		if not IsNull(rs2("NetPackagingWgt")) and Len(Trim(rs2("NetPackagingWgt"))) > 0 then matt1boxwgt = FormatNumber(rs2("NetPackagingWgt"),0,,,0)
	end if
	rs2.close
	set rs2=nothing
	...
	If not rs2.eof then
		...
		if not IsNull(rs2("packkg")) and Len(Trim(rs2("packkg"))) > 0 then matt2kg = FormatNumber(rs2("packkg"),0,,,0)
		if not IsNull(rs2("ProductWgt")) and Len(Trim(rs2("ProductWgt"))) > 0 then boxprodwgt1_2 = FormatNumber(rs2("ProductWgt"),0,,,0)
		if not IsNull(rs2("NetPackagingWgt")) and Len(Trim(rs2("NetPackagingWgt"))) > 0 then matt2boxwgt = FormatNumber(rs2("NetPackagingWgt"),0,,,0)
	end if
	rs2.close
	set rs2=nothing
end if
...
if not IsNull(rs2("packkg")) and Len(Trim(rs2("packkg"))) > 0 then base1kg = FormatNumber(rs2("packkg"),0,,,0)
if not IsNull(rs2("ProductWgt")) and Len(Trim(rs2("ProductWgt"))) > 0 then boxprodwgt3_1 = FormatNumber(rs2("ProductWgt"),0,,,0)
if not IsNull(rs2("NetPackagingWgt")) and Len(Trim(rs2("NetPackagingWgt"))) > 0 then base1boxwgt = FormatNumber(rs2("NetPackagingWgt"),0,,,0)
...
if not IsNull(rs2("packkg")) and Len(Trim(rs2("packkg"))) > 0 then base2kg = FormatNumber(rs2("packkg"),0,,,0)
if not IsNull(rs2("ProductWgt")) and Len(Trim(rs2("ProductWgt"))) > 0 then boxprodwgt3_2 = FormatNumber(rs2("ProductWgt"),0,,,0)
if not IsNull(rs2("NetPackagingWgt")) and Len(Trim(rs2("NetPackagingWgt"))) > 0 then base2boxwgt = FormatNumber(rs2("NetPackagingWgt"),0,,,0)
...
if not IsNull(rs2("packkg")) and Len(Trim(rs2("packkg"))) > 0 then topper1kg = FormatNumber(rs2("packkg"),0,,,0)
if not IsNull(rs2("ProductWgt")) and Len(Trim(rs2("ProductWgt"))) > 0 then boxprodwgt5_1 = FormatNumber(rs2("ProductWgt"),0,,,0)
if not IsNull(rs2("NetPackagingWgt")) and Len(Trim(rs2("NetPackagingWgt"))) > 0 then topper1boxwgt = FormatNumber(rs2("NetPackagingWgt"),0,,,0)
end if
rs2.close
set rs2=nothing
end if
...
if not IsNull(rs2("packkg")) and Len(Trim(rs2("packkg"))) > 0 then valance1kg = FormatNumber(rs2("packkg"),0,,,0)
if not IsNull(rs2("ProductWgt")) and Len(Trim(rs2("ProductWgt"))) > 0 then boxprodwgt6_1 = FormatNumber(rs2("ProductWgt"),0,,,0)
if not IsNull(rs2("NetPackagingWgt")) and Len(Trim(rs2("NetPackagingWgt"))) > 0 then valance1boxwgt = FormatNumber(rs2("NetPackagingWgt"),0,,,0)
end if
rs2.close
set rs2=nothing
end if
...
If not rs2.eof then
	...
	if not IsNull(rs2("packkg")) and Len(Trim(rs2("packkg"))) > 0 then legs1kg = FormatNumber(rs2("packkg"),0,,,0)
	if not IsNull(rs2("ProductWgt")) and Len(Trim(rs2("ProductWgt"))) > 0 then boxprodwgt7_1 = FormatNumber(rs2("ProductWgt"),0,,,0)
	if not IsNull(rs2("NetPackagingWgt")) and Len(Trim(rs2("NetPackagingWgt"))) > 0 then legs1boxwgt =FormatNumber(rs2("NetPackagingWgt"),0,,,0)
end if
rs2.close
set rs2=nothing
end if
...
if rs("headboardrequired")="y" and rs("wrappingid")=3 then
	...
	Set rs2 = getMysqlQueryRecordSet(sql, con)
	If not rs2.eof then
		if rs2("boxsize")<>"" then hb1boxsize=rs2("boxsize")
		
		if not IsNull(rs2("packkg")) and Len(Trim(rs2("packkg"))) > 0 then hb1kg = FormatNumber(rs2("packkg"),0,,,0)
		if not IsNull(rs2("ProductWgt")) and Len(Trim(rs2("ProductWgt"))) > 0 then boxprodwgt8_1 = FormatNumber(rs2("ProductWgt"),0,,,0)
		if not IsNull(rs2("NetPackagingWgt")) and Len(Trim(rs2("NetPackagingWgt"))) > 0 then hb1boxwgt =FormatNumber(rs2("NetPackagingWgt"),0,,,0)
		end if
	rs2.close
	set rs2=nothing
end if
...
<a href="/php/commercialinvoice.pdf?pno=<%=rs("purchase_no")%>&sid=<%=sid%>&loc=<%=loc%>&cid=<%=cid%>">Commercial Invoice</a> | <a href="/php/PickingNote.pdf?pn=<%=rs("purchase_no")%>" target="_blank">
...
<div id="delletter"><button onClick="window.open('/php/giftpack.pdf?pn=<%=rs("purchase_no")%>'); return false;">Delivery Letter</button></div>
<br /><br>
<%
if isNull(rs("bookeddeliverydate")) or rs("bookeddeliverydate")="" then
...
<select name="myjumpbox"
 OnChange="window.open(jump2.myjumpbox.options[selectedIndex].value)">
     ...
     <option value="php/Frames.pdf?pn=<%=rs("purchase_no")%>">Work Sheet - Frames
...
<table width="950" border="0" cellspacing="0" cellpadding="1">
  ...
  <tr>
    <% if not hideOrderType(con, rs("contact_no")) then %>
	    ...
	    <td valign="top"><%=getOrderType(con, rs)%></td>
    <% else %>
		<td colspan="2">&nbsp;</td>
    <% end if %>
    ...
    <tr>
      <td valign="top"><strong>Order Status:<br>
        <%Set rs3 = getMysqlUpdateRecordSet("Select * from QC_status where retiredCoreProducts='n' and mainstatusdropdowns='y'", con)%>
          <select name="mattressqc" id="mattressqc" onchange="resetExWorks('mattressqc', 'exworksdatematt'); checkProdDateCompleted(); disableMattressExWorksDate(); defaultAreaProductionDates(); checkFinishedDateCompleted('<%=mattressstatus%>')" >
...
<tr>
<td colspan="3" align="center" valign="bottom">Amend Commercial <br>
Invoice Container Manifest</td></tr>
<tr>
  <td colspan="3" align="center" valign="bottom">Crate Type:
<%Set rs3 = getMysqlQueryRecordSet("Select * from shippingbox WHERE (shippingBoxID > 15 and shippingBoxID < 21)", con)%>
<select name="mattressCrateType" id="mattressCrateType" onChange="checkSpecialCrateQty('1','mattressCrateType','matt_crateqty','mattspecialcrate2'); defaultCrateSize('1','matt1_packwidth','matt1_packheight','matt1_packdepth','','matt_crateqty', false); defaultCrateWeight('1','mattressCrateType','matt1cratewgt','matt_crateqty', false); defaultCompWeight('1', 'js_savoirmodel', 'js_matt1width', 'js_matt1length', 'crateprodwgt1_1', false); setTotalWeight('matt1cratewgt', 'crateprodwgt1_1', 'matt1_packkg'); defaultCrateSize('1','matt2_packwidth','matt2_packheight','matt2_packdepth','','matt_crateqty', false); defaultCrateWeight('1','mattressCrateType','matt2cratewgt','matt_crateqty', false); defaultCompWeight('1', 'js_savoirmodel', 'js_matt2width', 'js_matt2length', 'crateprodwgt1_2', false); setTotalWeight('matt2cratewgt', 'crateprodwgt1_2', 'matt2_packkg'); getChangedCrateSize('1','matt1_packwidth','matt1_packheight','matt1_packdepth','matt1_packkg','mattressCrateType','matt_crateqty'); getChangedCrateSize('1','matt2_packwidth','matt2_packheight','matt2_packdepth','matt2_packkg','mattressCrateType','matt_crateqty'); disablefield('mattressCrateType','matt1_packwidth','matt1_packheight','matt1_packdepth');  " >
 <%Do until rs3.eof%>
...
<tr><td align="center" valign="bottom">Packaging<br>Kg</td><td align="center" valign="bottom">Net<br>Kg</td><td align="center" valign="bottom">Total<br>Kg</td></tr>

<tr>
<td width="74" align="center" valign="top"><input name="matt1cratewgt" type="text" id="matt1cratewgt" size="5" maxlength="6" value="<%=matt1cratewgt%>" onChange="setTotalWeight('matt1cratewgt', 'crateprodwgt1_1', 'matt1_packkg');"></td>
<td width="74" align="center" valign="top"><input name="crateprodwgt1_1" type="text" id="crateprodwgt1_1" size="5" maxlength="6" value="<%=crateprodwgt1_1%>" onChange="setTotalWeight('matt1cratewgt', 'crateprodwgt1_1', 'matt1_packkg');"></td>
<td width="74" align="center" valign="top"><input name="matt1_packkg" type="text" id="matt1_packkg" size="5" maxlength="6" value="<%=matt1_packkg%>"></td>
</tr>
<!-- special crate no.2 -->
<tr class="mattspecialcrate2">
  <td colspan="3" align="center" valign="bottom">Crate Type:
<%Set rs3 = getMysqlQueryRecordSet("Select * from shippingbox WHERE (shippingBoxID = 20)", con)%>
<select name="mattressCrateType2" id="mattressCrateType2" onChange="defaultCrateSize('1','matt2_packwidth','matt2_packheight','matt2_packdepth','','matt_crateqty', false); defaultCrateWeight('1','mattressCrateType','matt2cratewgt','matt_crateqty', false); defaultCompWeight('1', 'js_savoirmodel', 'js_matt2width', 'js_matt2length', 'crateprodwgt1_2', false); setTotalWeight('matt2cratewgt', 'crateprodwgt1_2', 'matt2_packkg');" >
 <%Do until rs3.eof%>
...
<tr class="mattspecialcrate2">
<td align="center" valign="bottom">Packaging<br>Kg</td><td align="center" valign="bottom">Net<br>Kg</td><td align="center" valign="bottom">Total<br>Kg</td></tr>
<tr class="mattspecialcrate2">
<td width="74" align="center" valign="top"><input name="matt2cratewgt" type="text" id="matt2cratewgt" size="5" maxlength="6" value="<%=matt2cratewgt%>" onChange="setTotalWeight('matt2cratewgt', 'crateprodwgt1_2', 'matt2_packkg');"></td>
<td width="74" align="center" valign="top"><input name="crateprodwgt1_2" type="text" id="crateprodwgt1_2" size="5" maxlength="6" value="<%=crateprodwgt1_2%>" onChange="setTotalWeight('matt2cratewgt', 'crateprodwgt1_2', 'matt2_packkg');"></td>
<td width="74" align="center" valign="top"><input name="matt2_packkg" type="text" id="matt2_packkg" size="5" maxlength="6" value="<%=matt2kg%>"></td>
</tr>
  <td height="29" colspan="4" align="center">Crate Qty: 
    <label for="matt_crateqty"></label>
    <select name="matt_crateqty" id="matt_crateqty" onChange="checkSpecialCrateQty('1','mattressCrateType','matt_crateqty','mattspecialcrate2'); defaultCrateSize('1','matt1_packwidth','matt1_packheight','matt1_packdepth','','', false); defaultCrateWeight('1','mattressCrateType','matt1cratewgt','matt_crateqty', false); defaultCompWeight('1', 'js_savoirmodel', 'js_matt1width', 'js_matt1length', 'crateprodwgt1_1', false); setTotalWeight('matt1cratewgt', 'crateprodwgt1_1', 'matt1_packkg'); defaultCrateSize('1','matt2_packwidth','matt2_packheight','matt2_packdepth','','', false); defaultCrateWeight('1','mattressCrateType','matt2cratewgt','matt_crateqty', false); defaultCompWeight('1', 'js_savoirmodel', 'js_matt2width', 'js_matt2length', 'crateprodwgt1_2', false); setTotalWeight('matt2cratewgt', 'crateprodwgt1_2', 'matt2_packkg');" >
...
Invoice Container Manifest</td></tr>
<tr><td>Box Size</td><td>Packaging<br>Kg</td><td>Net<br>Kg</td><td>Total<br>Kg</td></tr>
...
%>
<select name="matt1boxsize" id="matt1boxsize" onChange="defaultBoxWeight('1','matt1boxsize','matt1boxwgt', false); setTotalWeight('matt1boxwgt', 'boxprodwgt1_1', 'matt1kg');" >
...
<td width="74" align="center" valign="top"><input name="matt1boxwgt" type="text" id="matt1boxwgt" size="5" maxlength="6" value="<%=matt1boxwgt%>" onChange="setTotalWeight('matt1boxwgt', 'boxprodwgt1_1', 'matt1kg');" /></td>
<td width="74" align="center" valign="top"><input name="boxprodwgt1_1" type="text" id="boxprodwgt1_1" size="5" maxlength="6" value="<%=boxprodwgt1_1%>" onChange="setTotalWeight('matt1boxwgt', 'boxprodwgt1_1', 'matt1kg');" /></td>
<td align="center"><input name="matt1kg" type="text" id="matt1kg" size="5" maxlength="6" value="<%=matt1kg%>" readonly></td>
</tr>
<%if left(rs("mattresstype"),3)="Zip" then
%>
<tr><td width="104" align="center" valign="top">
<select name="matt2boxsize" id="matt2boxsize" onChange="defaultBoxWeight('1','matt2boxsize','matt2boxwgt', false); setTotalWeight('matt2boxwgt', 'boxprodwgt1_2', 'matt2kg');" >
...
<td width="74" align="center" valign="top"><input name="matt2boxwgt" type="text" id="matt2boxwgt" size="5" maxlength="6" value="<%=matt2boxwgt%>" onChange="setTotalWeight('matt2boxwgt', 'boxprodwgt1_2', 'matt2kg');" /></td>
<td width="74" align="center" valign="top"><input name="boxprodwgt1_2" type="text" id="boxprodwgt1_2" size="5" maxlength="6" value="<%=boxprodwgt1_2%>" onChange="setTotalWeight('matt2boxwgt', 'boxprodwgt1_2', 'matt2kg');" /></td>
<td width="74" align="center" valign="top"><input name="matt2kg" type="text" id="matt2kg" size="5" maxlength="6" value="<%=matt2kg%>" readonly></td>
<td></td>
</tr>
<%end if%>
...
%>  <option value="<%=rs3("bay_no")%>" <%=selected(rs3("bay_no"), basebay) %>><%=rs3("bay_name")%></option>
	  ...
	  set rs3=nothing%></select></td>
            ...
            <tr>
              <td valign="top"><strong>Order Status:<br>
                 <%Set rs3 = getMysqlUpdateRecordSet("Select * from QC_status where retiredCoreProducts='n' and  mainstatusdropdowns='y'", con)%>
          <select name="baseqc" id="baseqc" onchange="resetExWorks('baseqc', 'exworksdatebase'); checkBaseProdDateCompleted(); disableBaseExWorksDate(); defaultAreaProductionDates(); checkBaseFinishedDateCompleted('<%=basestatus%>')" >" >
...
<%Set rs3 = getMysqlQueryRecordSet("Select * from shippingbox WHERE (shippingBoxID > 15 and shippingBoxID < 21)", con)%><select name="baseCrateType" id="baseCrateType" onChange="checkSpecialCrateQty('3','baseCrateType','base_crateqty','basespecialcrate2'); defaultCrateSize('3','base1_packwidth','base1_packheight','base1_packdepth','','base_crateqty', false); defaultCrateWeight('3','baseCrateType','base1cratewgt','base_crateqty', false); defaultCompWeight('3', 'js_basesavoirmodel', 'js_base1width', 'js_base1length', 'crateprodwgt3_1', false); setTotalWeight('base1cratewgt', 'crateprodwgt3_1', 'base1_packkg'); defaultCrateSize('3','base2_packwidth','base2_packheight','base2_packdepth','','base_crateqty', false); defaultCrateWeight('3','baseCrateType','base2cratewgt','base_crateqty', false); defaultCompWeight('3', 'js_basesavoirmodel', 'js_base2width', 'js_base2length', 'crateprodwgt3_2', false); setTotalWeight('base2cratewgt', 'crateprodwgt3_2', 'base2_packkg'); getChangedCrateSize('3','base1_packwidth','base1_packheight','base1_packdepth','base1_packkg','baseCrateType','base_crateqty'); getChangedCrateSize('3','base2_packwidth','base2_packheight','base2_packdepth','base2_packkg','baseCrateType','base_crateqty'); disablefield('baseCrateType','base1_packwidth','base1_packheight','base1_packdepth');  " >
...
<td width="74" align="center" valign="top"><input name="base1cratewgt" type="text" id="base1cratewgt" size="5" maxlength="6" value="<%=base1cratewgt%>" onChange="setTotalWeight('base1cratewgt', 'crateprodwgt3_1', 'base1_packkg');"></td>
<td width="74" align="center" valign="top"><input name="crateprodwgt3_1" type="text" id="crateprodwgt3_1" size="5" maxlength="6" value="<%=crateprodwgt3_1%>" onChange="setTotalWeight('base1cratewgt', 'crateprodwgt3_1', 'base1_packkg');"></td>
<td width="74" align="center" valign="top"><input name="base1_packkg" type="text" id="base1_packkg" size="5" maxlength="6" value="<%=base1_packkg%>" readonly></td>
</tr>
...
<td align="center" valign="bottom">Packaging<br>Kg</td><td align="center" valign="bottom">Net<br>Kg</td><td align="center" valign="bottom">Total<br>Kg</td></tr>
<tr class="basespecialcrate2">
<td width="74" align="center" valign="top"><input name="base2cratewgt" type="text" id="base2cratewgt" size="5" maxlength="6" value="<%=base2cratewgt%>" onChange="setTotalWeight('base2cratewgt', 'crateprodwgt3_2', 'base2_packkg');"></td>
<td width="74" align="center" valign="top"><input name="crateprodwgt3_2" type="text" id="crateprodwgt3_2"  size="5" maxlength="6" value="<%=crateprodwgt3_2%>" onChange="setTotalWeight('base2cratewgt', 'crateprodwgt3_2', 'base2_packkg');"></td>
<td width="74" align="center" valign="top"><input name="base2_packkg" type="text" id="base2_packkg" size="5" maxlength="6" value="<%=base2kg%>" readonly></td>
</tr>
<tr>
  <td colspan="4" align="center">Crate Qty:
    <label for="base_crateqty"></label>
    <select name="base_crateqty" id="base_crateqty" onChange="checkSpecialCrateQty('3','baseCrateType','base_crateqty','basespecialcrate2'); defaultCrateSize('3','base1_packwidth','base1_packheight','base1_packdepth','','', false); defaultCrateWeight('3','baseCrateType','base1cratewgt','base_crateqty', false); defaultCompWeight('3', 'js_basesavoirmodel', 'js_base1width', 'js_base1length', 'crateprodwgt3_1', false); setTotalWeight('base1cratewgt', 'crateprodwgt3_1', 'base1_packkg'); defaultCrateSize('3','base2_packwidth','base2_packheight','base2_packdepth','','', false); defaultCrateWeight('1','baseCrateType','base2cratewgt','base_crateqty', false); defaultCompWeight('3', 'js_basesavoirmodel', 'js_base2width', 'js_base2length', 'crateprodwgt3_2', false); setTotalWeight('base2cratewgt', 'crateprodwgt3_2', 'base2_packkg'); crateQtyChanged('3','base1_packwidth','base1_packheight','base1_packdepth','base1_packkg','baseCrateType','base_crateqty'); crateQtyChanged('3','base2_packwidth','base2_packheight','base2_packdepth','base2_packkg','baseCrateType','base_crateqty');" >
...
<td colspan="4" align="center" valign="bottom">Amend Commercial <br>
Invoice Container Manifest</td></tr>
<tr><td>Box Size</td><td>Packaging<br>Kg</td><td align="center">Net<br>Kg</td><td align="center">Total<br>Kg</td></tr>
<tr><td width="104" align="center" valign="top">
<%if base1width="" then base1width=rs("basewidth")
if base1length="" then base1length=rs("baselength")%>


<select name="base1boxsize" id="base1boxsize" onChange="defaultBoxWeight('3','base1boxsize','base1boxwgt', false); setTotalWeight('base1boxwgt', 'boxprodwgt3_1', 'base1kg');" >
...
<td width="74" align="center" valign="top"><input name="base1boxwgt" type="text" id="base1boxwgt" size="5" maxlength="6" value="<%=base1boxwgt%>" onChange="setTotalWeight('base1boxwgt', 'boxprodwgt3_1', 'base1kg');" /></td>
<td width="74" align="center" valign="top"><input name="boxprodwgt3_1" type="text" id="boxprodwgt3_1" size="5" maxlength="6" value="<%=boxprodwgt3_1%>" onChange="setTotalWeight('base1boxwgt', 'boxprodwgt3_1', 'base1kg');"></td>
<td width="74" align="center" valign="top"><input name="base1kg" type="text" id="base1kg" size="5" maxlength="6" value="<%=base1kg%>" readonly></td>
<td align="center">
</td>
</tr>
...
<select name="base2boxsize" id="base2boxsize" onChange="defaultBoxWeight('3','base2boxsize','base2boxwgt', false); setTotalWeight('base2boxwgt', 'boxprodwgt3_2', 'base2kg');" >
...
<td width="74" align="center" valign="top"><input name="base2boxwgt" type="text" id="base2boxwgt" size="5" maxlength="6" value="<%=base2boxwgt%>" onChange="setTotalWeight('base2boxwgt', 'boxprodwgt3_2', 'base2kg');" ></td>
<td width="74" align="center" valign="top"><input name="boxprodwgt3_2" type="text" id="boxprodwgt3_2" size="5" maxlength="6" value="<%=boxprodwgt3_2%>" onChange="setTotalWeight('base2boxwgt', 'boxprodwgt3_2', 'base2kg');" ></td>
<td width="74" align="center" valign="top"><input name="base2kg" type="text" id="base2kg" size="5" maxlength="6" value="<%=base2kg%>" readonly></td>
<td></td>
</tr>
<%end if%>
...
%>  <option value="<%=rs3("bay_no")%>" <%=selected(rs3("bay_no"), topperbay) %>><%=rs3("bay_name")%></option>
      ...
      <tr>
        <td valign="top"><strong>Order Status:<br>
           <%Set rs3 = getMysqlUpdateRecordSet("Select * from QC_status where retiredCoreProducts='n' and mainstatusdropdowns='y'", con)%>
          <select name="topperstatus" id="topperstatus" onchange="resetExWorks('topperstatus', 'exworksdatetopper'); checkTopperProdDateCompleted(); disableTopperExWorksDate(); defaultAreaProductionDates(); checkTopperFinishedDateCompleted('<%=topperstatus%>')" >
...
<tr>
  <td colspan="4" align="center" valign="bottom">Crate Type:
<%Set rs3 = getMysqlQueryRecordSet("Select * from shippingbox WHERE (shippingBoxID > 15 and shippingBoxID < 21)", con)%><select name="topperCrateType" id="topperCrateType" 

onChange="checkSpecialCrateQty('5','topperCrateType','topper_crateqty','topperspecialcrate2'); defaultCrateSize('5','topper1_packwidth','topper1_packheight','topper1_packdepth','','topper_crateqty', false); defaultCrateWeight('5','topperCrateType','topper1cratewgt','toppere_crateqty', false); defaultCompWeight('5', 'js_toppertype', 'js_topper1width', 'js_topper1length', 'crateprodwgt5_1', false); setTotalWeight('topper1cratewgt', 'crateprodwgt5_1', 'topper1_packkg'); getChangedCrateSize('5','topper1_packwidth','topper1_packheight','topper1_packdepth','topper1_packkg','topperCrateType','topper_crateqty'); disablefield('topperCrateType','topper1_packwidth','topper1_packheight','topper1_packdepth'); checkSpecialCrateQty('5','topperCrateType','topper_crateqty','topperspecialcrate2');" >
...
<td align="center" valign="bottom">Packaging<br>Kg</td><td align="center" valign="bottom">Net<br>Kg</td><td align="center" valign="bottom">Total<br>Kg</td></tr>
<tr>
<td width="74" align="center" valign="top"><input name="topper1cratewgt" type="text" id="topper1cratewgt" size="5" maxlength="6" value="<%=topper1cratewgt%>" onChange="setTotalWeight('topper1cratewgt', 'crateprodwgt5_1', 'topper1_packkg');"></td>
<td width="74" align="center" valign="top"><input name="crateprodwgt5_1" type="text" id="crateprodwgt5_1" size="5" maxlength="6"  value="<%=crateprodwgt5_1%>" onChange="setTotalWeight('topper1cratewgt', 'crateprodwgt5_1', 'topper1_packkg');"></td>
<td width="74" align="center" valign="top"><input name="topper1_packkg" type="text" id="topper1_packkg" size="5" maxlength="6" value="<%=topper1_packkg%>" readonly></td>
</tr>
...
<tr>
  <td colspan="3" align="center">Crate Qty: 
...
<td colspan="3" align="center" valign="bottom">Amend Commercial <br>
Invoice Container Manifest</td></tr>
<tr><td>Box Size</td><td>Packaging kg</td><td>Net kg</td><td>Total kg</td></tr>
<tr><td width="104" align="center" valign="top">
<%if topper1width="" then topper1width=rs("topperwidth")
if topper1length="" then topper1length=rs("topperlength")
%>

<select name="topper1boxsize" id="topper1boxsize" onChange="defaultBoxWeight('5','topper1boxsize','topper1boxwgt', false); setTotalWeight('topper1boxwgt', 'boxprodwgt5_1', 'topper1kg');" >
...
<td width="74" align="center" valign="top"><input name="topper1boxwgt" type="text" id="topper1boxwgt" size="5" maxlength="6" value="<%=topper1boxwgt%>" onChange="setTotalWeight('topper1boxwgt', 'boxprodwgt5_1', 'topper1kg');" ></td>
<td width="74" align="center" valign="top"><input name="boxprodwgt5_1" type="text" id="boxprodwgt5_1" size="5" maxlength="6" value="<%=boxprodwgt5_1%>" onChange="setTotalWeight('topper1boxwgt', 'boxprodwgt5_1', 'topper1kg');" ></td>
<td width="74" align="center" valign="top"><input name="topper1kg" type="text" id="topper1kg" size="5" maxlength="6" value="<%=topper1kg%>" readonly></td><td>
</tr>
</table></td></tr>
...
%>  <option value="<%=rs3("bay_no")%>" <%=selected(rs3("bay_no"), headboardbay) %>><%=rs3("bay_name")%></option>
    ...
    <tr>
      <td valign="top"><strong>Order Status:<br>
         <%Set rs3 = getMysqlUpdateRecordSet("Select * from QC_status where retiredCoreProducts='n' and mainstatusdropdowns='y'", con)%>
          <select name="headboardstatus" id="headboardstatus" onchange="resetExWorks('headboardstatus', 'exworksdatehb'); disableHeadboardExWorksDate(); checkHeadboardProdDateCompleted(); defaultAreaProductionDates(); checkHBFinishedDateCompleted('<%=headboardstatus%>')" >
...
<select name="hb1boxsize" id="hb1boxsize" onChange="defaultBoxWeight('8','hb1boxsize','hb1boxwgt', false); setTotalWeight('hb1boxwgt', 'boxprodwgt8_1', 'hb1kg');" >
	<option value="" <%=selected("", hb1boxsize) %>> </option>
	<option value="Small" <%=selected("Small", hb1boxsize) %>>Small</option>
	<option value="Medium" <%=selected("Medium", hb1boxsize) %>>Medium</option>
	<option value="Large" <%=selected("Large", hb1boxsize) %>>Large</option>
</select></td>

<td width="74" align="center" valign="top"><input name="hb1boxwgt" type="text" id="hb1boxwgt" size="5" maxlength="6" value="<%=hb1boxwgt%>" onChange="setTotalWeight('hb1boxwgt', 'boxprodwgt8_1', 'hb1kg');" /></td>
<td width="74" align="center" valign="top"><input name="boxprodwgt8_1" type="text" id="boxprodwgt8_1" size="5" maxlength="6" value="<%=boxprodwgt8_1%>" onChange="setTotalWeight('hb1boxwgt', 'boxprodwgt8_1', 'hb1kg');" /></td>
<td align="center"><input name="hb1kg" type="text" id="hb1kg" size="5" maxlength="6" value="<%=hb1kg%>" readonly></td>
...
<tr>
  <td colspan="3" align="center" valign="bottom">Crate Type:
<%Set rs3 = getMysqlQueryRecordSet("Select * from shippingbox WHERE (shippingBoxID > 15 and shippingBoxID < 21)", con)%>
<select name="hbCrateType" id="hbCrateType" onchange="checkSpecialCrateQty('8','hbCrateType','hb_crateqty','hbspecialcrate2'); defaultCrateSize('8','hb1_packwidth','hb1_packheight','hb1_packdepth','','hb_crateqty', false); defaultCrateWeight('8','hbCrateType','hb1cratewgt','hb_crateqty', false); defaultCompWeight('8', 'js_headboardstyle', 'js_hb1width', '', 'crateprodwgt8_1', false); setTotalWeight('hb1cratewgt', 'crateprodwgt8_1', 'hb1_packkg'); defaultCrateWeight('8','hbCrateType','hb2cratewgt','hb_crateqty', false); defaultCompWeight('8', 'js_headboardstyle', 'js_hb1width', '', 'crateprodwgt8_2', false); setTotalWeight('hb2cratewgt', 'crateprodwgt8_2', 'hb2_packkg');">
 <%Do until rs3.eof%>
    <option value="<%=rs3("sName")%>" <%=selected(rs3("sName"), hbCrateType)%>><%=rs3("sName")%></option>
 <%rs3.movenext
 loop
 rs3.close
 set rs3=nothing %>
...
<td align="center" valign="bottom">Packaging<br>Kg</td><td align="center" valign="bottom">Net<br>Kg</td><td align="center" valign="bottom">Total<br>Kg</td></tr>
<tr>
<td width="74" align="center" valign="top"><input name="hb1cratewgt" type="text" id="hb1cratewgt" size="5" maxlength="6" value="<%=hb1cratewgt%>" onChange="setTotalWeight('hb1cratewgt', 'crateprodwgt8_1', 'hb1_packkg');"></td>
<td width="74" align="center" valign="top"><input name="crateprodwgt8_1" type="text" id="crateprodwgt8_1" size="5" maxlength="6" value="<%=crateprodwgt8_1%>" onChange="setTotalWeight('hb1cratewgt', 'crateprodwgt8_1', 'hb1_packkg');"></td>
<td width="74" align="center" valign="top"><input name="hb1_packkg" type="text" id="hb1_packkg" size="5" maxlength="6" value="<%=hb1_packkg%>" readonly></td>
</tr>
...
<td align="center" valign="bottom">Packaging<br>Kg</td><td align="center" valign="bottom">Net<br>Kg</td><td align="center" valign="bottom">Total<br>Kg</td></tr>
<tr class="hbspecialcrate2">
<td width="74" align="center" valign="top"><input name="hb2cratewgt" type="text" id="hb2cratewgt" size="5" maxlength="6" value="<%=hb2cratewgt%>" onChange="setTotalWeight('hb2cratewgt', 'crateprodwgt8_2', 'hb2_packkg');"></td>
<td width="74" align="center" valign="top"><input name="crateprodwgt8_2" type="text" id="crateprodwgt8_2" size="5" maxlength="6" value="<%=crateprodwgt8_2%>" onChange="setTotalWeight('hb2cratewgt', 'crateprodwgt8_2', 'hb2_packkg');"></td>
<td width="74" align="center" valign="top"><input name="hb2_packkg" type="text" id="hb2_packkg" size="5" maxlength="6" value="<%=hb2_packkg%>" readonly></td>
</tr>
<tr>
  <td colspan="3" align="center">Crate Qty:
    <label for="hb_crateqty"></label>
	<select name="hb_crateqty" id="hb_crateqty" onChange="checkSpecialCrateQty('8','hbCrateType','hb_crateqty','hbspecialcrate2'); defaultCrateSize('8','hb1_packwidth','hb1_packheight','hb1_packdepth','','', false); defaultCrateWeight('8','hbCrateType','hb1cratewgt','hb_crateqty', false); defaultCompWeight('8', 'js_headboardstyle', 'js_hb1width', '', 'crateprodwgt8_1', false); setTotalWeight('hb1cratewgt', 'crateprodwgt8_1', 'hb1_packkg'); defaultCrateSize('8','hb2_packwidth','hb2_packheight','hb2_packdepth','','', false); defaultCrateWeight('8','hbCrateType','hb2cratewgt','hb_crateqty', false); defaultCompWeight('8', 'js_headboardstyle', 'js_hb1width', '', 'crateprodwgt8_2', false); setTotalWeight('hb2cratewgt', 'crateprodwgt8_2', 'hb2_packkg'); crateQtyChanged('8','hb1_packwidth','hb1_packheight','hb1_packdepth','hb1_packkg','hbCrateType','hb_crateqty'); crateQtyChanged('8','hb2_packwidth','hb2_packheight','hb2_packdepth','hb2_packkg','hbCrateType','hb_crateqty');" >
...
Invoice Container Manifest</td></tr>
<tr><td colspan="3" align="center" valign="bottom"> Dimensions (cm)</td>
<td align="center" valign="bottom">Product<br>Kg</td>
</tr>
<tr>
...
%>
  ...
  <tr>
	...
	%>
    <%=totalLegQty%>
	<%if (rs("headboardrequired")="y" and Not IsNull(rs("headboardlegqty"))) then 
		totalLegQty=totalLegQty + cint(rs("headboardlegqty"))
	end if%>&nbsp;</td>
  </tr>
...
%>  <option value="<%=rs3("bay_no")%>" <%=selected(rs3("bay_no"), legsbay) %>><%=rs3("bay_name")%></option>
    ...
    <tr>
      <td valign="top"><strong>Order Status:<br>
         <%Set rs3 = getMysqlUpdateRecordSet("Select * from QC_status where retiredCoreProducts='n' and mainstatusdropdowns='y'", con)%>
          <select name="legsstatus" id="legsstatus" onChange="resetExWorks('legsstatus', 'exworksdatelegs'); checkLegsProdDateCompleted(); disableLegsExWorksDate(); defaultAreaProductionDates(); checkLegsFinishedDateCompleted('<%=legsstatus%>')" >
...
<td><table border="0" align="left" cellpadding="1" cellspacing="2" class="packtableborder">
<tr>
<td colspan="3" align="center" valign="bottom"><p>Amend Commercial <br>
  Invoice Container Manifest</p></td></tr>
<tr class="nolegcrate">
  <td colspan="2" align="center" valign="bottom">No crate</td>
...
<td align="center" valign="bottom">Packaging<br>Kg</td><td align="center" valign="bottom">Net<br>Kg</td><td align="center" valign="bottom">Total<br>Kg</td></tr>
<tr class="hidelegcrate">
<td width="74" align="center" valign="top"><input name="legs1cratewgt" type="text" id="legs1cratewgt" size="5" maxlength="6" value="<%=legs1cratewgt%>" onChange="setTotalWeight('legs1cratewgt', 'crateprodwgt7_1', 'legs1_packkg');"></td>
<td width="74" align="center" valign="top"><input name="crateprodwgt7_1" type="text" id="crateprodwgt7_1" size="5" maxlength="6" value="<%=crateprodwgt7_1%>" onChange="setTotalWeight('legs1cratewgt', 'crateprodwgt7_1', 'legs1_packkg');"></td>
<td width="74" align="center" valign="top"><input name="legs1_packkg" type="text" id="legs1_packkg" size="5" maxlength="6" value="<%=legs1_packkg%>" readonly></td>
</tr>

<tr class="hidelegcrate">
  <td colspan="3" align="center" valign="bottom">Crate Type:
<%Set rs3 = getMysqlQueryRecordSet("Select * from shippingbox WHERE (shippingBoxID > 15 and shippingBoxID < 21)", con)%><select name="legsCrateType" id="legsCrateType" onChange="checkSpecialCrateQty('7','legsCrateType','legs_crateqty','legsspecialcrate2'); defaultCrateSize('7','legs1_packwidth','legs1_packheight','legs1_packdepth','','legs_crateqty', false); 
defaultCrateWeight('7','legsCrateType','legs1cratewgt','legs_crateqty', false); defaultCompWeight('7', 'js_legstyle', 'legs1boxsize', '', 'crateprodwgt7_1', false); setTotalWeight('legs1cratewgt', 'crateprodwgt7_1', 'legs1_packkg'); getChangedCrateSize('7','legs1_packwidth','legs1_packheight','legs1_packdepth','legs1_packkg','legsCrateType','legs_crateqty'); disablefield('legsCrateType','legs1_packwidth','legs1_packheight','legs1_packdepth');  " >
...
<td align="center" valign="bottom">Packaging<br>Kg</td><td align="center" valign="bottom">Net<br>Kg</td><td align="center" valign="bottom">Total<br>Kg</td></tr>
<tr class="legsspecialcrate2">
<td width="74" align="center" valign="top"><input name="legs2cratewgt" type="text" id="legs2cratewgt" size="5" maxlength="6" value="<%=legs2cratewgt%>" onChange="setTotalWeight('legs2cratewgt', 'crateprodwgt7_2', 'legs2_packkg');"></td>
<td width="74" align="center" valign="top"><input name="crateprodwgt7_2" type="text" id="crateprodwgt7_2" size="5" maxlength="6"></td>
<td width="74" align="center" valign="top"><input name="legs2_packkg" type="text" id="legs2_packkg" size="5" maxlength="6" value="<%=legs2_packkg%>" onChange="setTotalWeight('legs2cratewgt', 'crateprodwgt7_2', 'legs2_packkg');"></td>
</tr>
<tr class="legsspecialcrate2">
  <td colspan="3" align="center" valign="bottom">Crate Type:
...
<tr class="hidelegcrate">
  <td colspan="3" align="center">Crate Qty:
    ...
    <select name="legs_crateqty" id="legs_crateqty" onChange="checkSpecialCrateQty('7','legsCrateType','legs_crateqty','legsspecialcrate2'); defaultCrateSize('7','legs1_packwidth','legs1_packheight','legs1_packdepth','legsCrateType','legs_crateqty', false); defaultCrateWeight('7','legsCrateType','legs1cratewgt','legs_crateqty', false); defaultCompWeight('7', 'js_legstyle', 'legs1boxsize', '', 'crateprodwgt7_1', false); setTotalWeight('legs1cratewgt', 'crateprodwgt7_1', 'legs1_packkg');  crateQtyChanged('7','legs1_packwidth','legs1_packheight','legs1_packdepth','legs1_packkg','legsCrateType','legs_crateqty'); ">
...
<tr class="legspackedwithshow">
  <td width="145" colspan="3" align="center">Legs Packed with:<br>
...
<tr><td>Box Size</td><td>Packaging kg</td><td>Net kg</td><td>Total kg</td></tr>
<tr><td width="104" align="center" valign="top">
<select name="legs1boxsize" id="legs1boxsize" onChange="defaultBoxWeight('7','legs1boxsize','legs1boxwgt',false); setTotalWeight('legs1boxwgt','boxprodwgt7_1', 'legs1kg');" >
...
<td width="74" align="center" valign="top"><input name="legs1boxwgt" type="text" id="legs1boxwgt" size="5" maxlength="6" value="<%=legs1boxwgt%>" onChange="setTotalWeight('legs1boxwgt', 'boxprodwgt7_1', 'legs1kg');"></td>
<td width="74" align="center" valign="top"><input name="boxprodwgt7_1" type="text" id="boxprodwgt7_1" size="5" maxlength="6" value="<%=boxprodwgt7_1%>" onChange="setTotalWeight('legs1boxwgt', 'boxprodwgt7_1', 'legs1kg');" /></td>
<td width="71" align="center" valign="top"><input name="legs1kg" type="text" id="legs1kg" size="5" maxlength="6" value="<%=legs1kg%>" readonly /></td>
...
%>  <option value="<%=rs3("bay_no")%>" <%=selected(rs3("bay_no"), valancebay) %>><%=rs3("bay_name")%></option>
	  ...
	  set rs3=nothing%></select></td>
        ...
        <tr>
          <td valign="top"><strong>Order Status:<br>
             <%Set rs3 = getMysqlUpdateRecordSet("Select * from QC_status where retiredCoreProducts='n' and mainstatusdropdowns='y'", con)%>
          <select name="valancestatus" id="valancestatus" onchange="resetExWorks('valancestatus', 'exworksdatevalance'); checkValanceProdDateCompleted(); disableValanceExWorksDate(); defaultAreaProductionDates(); checkValanceFinishedDateCompleted('<%=valancestatus%>')" >
             <%do until rs3.eof
...
<td><table border="0" align="left" cellpadding="1" cellspacing="2" class="packtableborder">
<tr>
<td colspan="5" align="center" valign="bottom">Amend Commercial <br>
Invoice Container Manifest</td></tr>
...
<tr class="hidevalancecrate">
  <td colspan="4" align="center" valign="bottom">Crate Type:
<%Set rs3 = getMysqlQueryRecordSet("Select * from shippingbox WHERE (shippingBoxID > 15 and shippingBoxID < 21)", con)%><select name="valanceCrateType" id="valanceCrateType" 
onChange="checkSpecialCrateQty('6','valanceCrateType','valance_crateqty','valancespecialcrate2'); defaultCrateSize('6','valance1_packwidth','valance1_packheight','valance1_packdepth','','valance_crateqty', false); defaultCrateWeight('6','valanceCrateType','valance1cratewgt','valance_crateqty', false); defaultCompWeight('6', 'js_valance', 'js_valance1width', 'js_valance1length', 'crateprodwgt6_1', false); setTotalWeight('valance1cratewgt', 'crateprodwgt6_1', 'valance1_packkg'); getChangedCrateSize('6','valance1_packwidth','valance1_packheight','valance1_packdepth','valance1_packkg','valanceCrateType','valance_crateqty'); disablefield('valanceCrateType','valance1_packwidth','valance1_packheight','valance1_packdepth'); checkSpecialCrateQty('6','valanceCrateType','valance_crateqty','valancespecialcrate2');" >
...
<td align="center"><label for="valance1_packwidth"></label>
...
<td align="center" valign="bottom">Packaging<br>Kg</td><td align="center" valign="bottom">Net<br>Kg</td><td align="center" valign="bottom">Total<br>Kg</td></tr>
<tr class="hidevalancecrate">
<td width="74" align="center" valign="top"><input name="valance1cratewgt" type="text" id="valance1cratewgt" size="5" maxlength="6" value="<%=valance1cratewgt%>" onChange="setTotalWeight('valance1cratewgt', 'crateprodwgt6_1', 'valance1_packkg');"></td>
<td width="74" align="center" valign="top"><input name="crateprodwgt6_1" type="text" id="crateprodwgt6_1" size="5" maxlength="6" value="<%=crateprodwgt6_1%>" onChange="setTotalWeight('valance1cratewgt', 'crateprodwgt6_1', 'valance1_packkg');"></td>
<td width="74" align="center" valign="top"><input name="valance1_packkg" type="text" id="valance1_packkg" size="5" maxlength="6" value="<%=valance1_packkg%>" readonly /></td>
</tr>
...
<td align="center" valign="bottom">Packaging<br>Kg</td><td align="center" valign="bottom">Net<br>Kg</td><td align="center" valign="bottom">Total<br>Kg</td></tr>
<tr class="valancespecialcrate2">
<td width="74" align="center" valign="top"><input name="valance2cratewgt" type="text" id="valance2cratewgt" size="5" maxlength="6" value="<%=valance2cratewgt%>" onChange="setTotalWeight('valance2cratewgt', 'crateprodwgt6_2', 'valance2_packkg');"></td>
<td width="74" align="center" valign="top"><input name="crateprodwgt6_2" type="text" id="crateprodwgt6_2" size="5" maxlength="6" onChange="setTotalWeight('valance2cratewgt', 'crateprodwgt6_2', 'valance2_packkg');"></td>
<td width="74" align="center" valign="top"><input name="valance2_packkg" type="text" id="valance2_packkg" size="5" maxlength="6" value="<%=valance2_packkg%>"></td>
</tr>

<tr class="hidevalancecrate">
  <td colspan="5" align="center">Crate Qty:
...
<tr class="valancepackedwithshow">
  <td colspan="3" align="center">Valance Packed with:<br>
...
<td width="74" align="center" valign="top"></td>
  </tr>
<%rs3.close
set rs3=nothing%>
...
<tr><td>Box Size</td><td>Packaging kg</td><td>Net kg</td><td>Total kg</td></tr>
<tr><td width="104" align="center" valign="top">
<select name="valance1boxsize" id="valance1boxsize"  onChange="defaultBoxWeight('6','valance1boxsize','valance1boxwgt', false); setTotalWeight('valance1boxwgt', 'boxprodwgt6_1', 'valance1kg');" >
...
<td width="74" align="center" valign="top"><input name="valance1boxwgt" type="text" id="valance1boxwgt" size="5" maxlength="6" value="<%=valance1boxwgt%>" onChange="setTotalWeight('valance1boxwgt', 'boxprodwgt6_1', 'valance1kg');" /></td>
<td width="74" align="center" valign="top"><input name="boxprodwgt6_1" type="text" id="boxprodwgt6_1" size="5" maxlength="6" value="<%=boxprodwgt6_1%>" onChange="setTotalWeight('valance1boxwgt', 'boxprodwgt6_1', 'valance1kg');" /></td>
<td width="71" align="center" valign="top"><input name="valance1kg" type="text" id="valance1kg" size="5" maxlength="6" value="<%=valance1kg%>" readonly /></td>
</tr>
</table></td></tr>
<%else%>
...
<tr>
  <td align="center">Valance Packed with:<br>
...
No.&nbsp;<%=i%></b></td>
		...
		<tr>
		  ...
		  <td align="left">Purchase <br>
		    Order Date</td>
		  <td align="left">ETA<br>
		    Date</td>
		  <td align="left">Received<br>
		    Date</td>
		  <td align="left">Checked &amp; <br>
		    Picked Date</td>
		  <td align="left">Special Instructions<br>
/ Details</td>
<td align="left">Qty to <br>
follow</td>
<td align="left">Delivered<br>Date</td>
<td align="left" class="tariffcode">Tariff Code<br></td>
<td align="left" class="acc_exworksdate">Ex-Works Date<br></td>
</tr>
...
<br>
 ...
 <td valign="top" class="tariffcode"><input name="acc_packtariffcode<%=i%>" type="text" id="acc_packtariffcode<%=i%>" value="<%=acc_packtariffcode%>" size="10" /><br></td>
 
 <td valign="top">
 <div id="exworksacc_<%=i%>">
 <%=makeAccessoriesExworksDateDropdown(con, i)%>
 </div>
      ...
      <tr class="accexpboxonly">
        ...
        <td colspan="7" valign="top">Packed With</td>
      </tr>
      <tr class="accexpboxonly">
          <td valign="top">&nbsp;</td>
          <td valign="top"><strong>
...
<tr bgcolor="#666666">
<td colspan="12" valign="top" align="right">
...
<%if rs("accessoriesrequired")="y" then
	currentexworksdate=""%>
	<%if retrieveUserLocation()=1 or userHasRoleInList("ORDERDETAILS_VIEWER") then%>
		<p align="right"><strong>Delivery Method:<br></strong></p>
		<p align="right"><strong>
		<%Set rs3 = getMysqlUpdateRecordSet("Select * from DeliveryMethod", con)%>
		<select name="accdelmethod" id="accdelmethod" onchange="showHideExWorksAccessories('exworksacc_', 'accdelmethod', 'exworksdateacc_')">
			<option value="">--</option>
			<%do until rs3.eof%>
...
<tr class="hideacccrate">
  <td colspan="5" align="center" valign="bottom">Crate Type:
<%Set rs3 = getMysqlQueryRecordSet("Select * from shippingbox WHERE (shippingBoxID > 15 and shippingBoxID < 21)", con)%><select name="accCrateType" id="accCrateType" onChange="checkSpecialCrateQty('9','accCrateType','acc_crateqty','accspecialcrate2'); defaultCrateSize('9','acc1_packwidth','acc1_packheight','acc1_packdepth','','acc_crateqty', false);  defaultCrateWeight('9','accCrateType','acc1cratewgt','acc_crateqty', false); setTotalWeight('acc1cratewgt', 'crateprodwgt9_1', 'acc1_packkg'); defaultCrateWeight('9','accCrateType','acc2cratewgt','acc_crateqty', false);  setTotalWeight('acc2cratewgt', 'crateprodwgt9_2', 'acc2_packkg'); disablefield('accCrateType','acc1_packwidth','acc_packheight','acc1_packdepth'); checkSpecialCrateQty('9','accCrateType','acc_crateqty','accspecialcrate2'); " >
...
<td align="center" valign="bottom">Packaging<br>Kg</td><td align="center" valign="bottom">Net<br>Kg</td><td align="center" valign="bottom">Total<br>Kg</td></tr>
<tr class="hideacccrate">
<td width="74" align="center" valign="top"><input name="acc1cratewgt" type="text" id="acc1cratewgt" size="5" maxlength="6" value="<%=acc1cratewgt%>" onChange="setTotalWeight('acc1cratewgt', 'crateprodwgt9_1', 'acc1_packkg');"></td>
<td width="74" align="center" valign="top"><input name="crateprodwgt9_1" type="text" id="crateprodwgt9_1" size="5" maxlength="6" value="<%=crateprodwgt9_1%>" onChange="setTotalWeight('acc1cratewgt', 'crateprodwgt9_1', 'acc1_packkg');"></td>
<td width="74" align="center" valign="top"><input name="acc1_packkg" type="text" id="acc1_packkg" size="5" maxlength="6" value="<%=acc1_packkg%>" readonly></td>
</tr>
<!-- special crate no.2 -->
...
<td align="center" valign="bottom">Packaging<br>Kg</td><td align="center" valign="bottom">Net<br>Kg</td><td align="center" valign="bottom">Total<br>Kg</td></tr>
<tr class="accspecialcrate2">
<td width="74" align="center" valign="top"><input name="acc2cratewgt" type="text" id="acc2cratewgt" size="5" maxlength="6" value="<%=acc2cratewgt%>" onChange="setTotalWeight('acc2cratewgt', 'crateprodwgt9_2', 'acc2_packkg');"></td>
<td width="74" align="center" valign="top"><input name="crateprodwgt9_2" type="text" id="crateprodwgt9_2" size="5" maxlength="6"  value="<%=crateprodwgt9_2%>" onChange="setTotalWeight('acc2cratewgt', 'crateprodwgt9_2', 'acc2_packkg');"></td>
<td width="74" align="center" valign="top"><input name="acc2_packkg" type="text" id="acc2_packkg" size="5" maxlength="6" value="<%=acc2_packkg%>" readonly></td>
</tr>
<tr class="hideacccrate">
  <td colspan="3" align="center">Crate Qty:
    <label for="acc_crateqty"></label>
    <select name="acc_crateqty" id="acc_crateqty" onChange="checkSpecialCrateQty('8','accCrateType','acc_crateqty','accspecialcrate2'); defaultCrateSize('9','acc1_packwidth','acc1_packheight','acc1_packdepth','','', false); 
defaultCrateWeight('9','accCrateType','acc1cratewgt','acc_crateqty', false); setTotalWeight('acc1cratewgt', 'crateprodwgt9_1', 'acc1_packkg'); defaultCrateSize('9','acc2_packwidth','acc2_packheight','acc2_packdepth','','', false); defaultCrateWeight('9','accCrateType','acc2cratewgt','acc_crateqty', false); setTotalWeight('acc2cratewgt', 'crateprodwgt9_2', 'acc2_packkg'); crateQtyChanged('9','acc1_packwidth','acc1_packheight','acc1_packdepth','acc1_packkg','accCrateType','acc_crateqty'); crateQtyChanged('9','acc2_packwidth','acc2_packheight','acc2_packdepth','acc2_packkg','accCrateType','acc_crateqty'); disablefield('accCrateType','acc1_packwidth','acc_packheight','acc1_packdepth'); checkSpecialCrateQty('9','accCrateType','acc_crateqty','accspecialcrate2');"  >
...
<tr class="accpackedwithshow">
  <td width="196" colspan="5" align="center">Packed with:<br>
...
<!-- hidden fields -->
<input type="hidden" name="js_savoirmodel" id="js_savoirmodel" value="<%=rs("savoirmodel")%>" />
<% if mattwidthdivided<>"" and zippedpair<>"n" then %>
	<input type="hidden" name="js_mattresscount" id="js_mattresscount" value="2" />
	<input type="hidden" name="js_matt1width" id="js_matt1width" value="<%=mattwidthdivided%>" />
	<input type="hidden" name="js_matt2width" id="js_matt2width" value="<%=mattwidthdivided%>" />
<% else %>
	<input type="hidden" name="js_mattresscount" id="js_mattresscount" value="1" />
	<input type="hidden" name="js_matt1width" id="js_matt1width" value="<%=mattwidthdivided%>" />
	<input type="hidden" name="js_matt2width" id="js_matt2width" value="" />
<% end if %>
<% if mattlengthdivided<>"" and zippedpair<>"n" then %>
	<input type="hidden" name="js_matt1length" id="js_matt1length" value="<%=mattlengthdivided%>" />
	<input type="hidden" name="js_matt2length" id="js_matt2length" value="<%=mattlengthdivided%>" />
<% else %>
	<input type="hidden" name="js_matt1length" id="js_matt1length" value="<%=mattlengthdivided%>" />
	<input type="hidden" name="js_matt2length" id="js_matt2length" value="" />
<% end if %>
...
<input type="hidden" name="js_base1length" id="js_base1length" value="<%=base1length%>" />
<%end if
if right(topper1length,2)="cm" then  topper1length= left(rs("topperlength"), len(rs("topperlength"))-2)
if right(topper1width,2)="cm" then  topper1width= left(rs("topperwidth"), len(rs("topperwidth"))-2)
%>

<input type="hidden" name="js_toppertype" id="js_toppertype" value="<%=rs("toppertype")%>" />
<input type="hidden" name="js_topper1width" id="js_topper1width" value="<%=topper1width%>" />
<input type="hidden" name="js_topper1length" id="js_topper1length" value="<%=topper1length%>" />

<input type="hidden" name="js_headboardstyle" id="js_headboardstyle" value="<%=rs("headboardstyle")%>" />
<input type="hidden" name="js_hb1width" id="js_hb1width" value="<%=hb_packwidth%>" />
<input type="hidden" name="js_hb1height" id="js_hb1height" value="<%=hb_packheight%>" />

<input type="hidden" name="js_legstyle" id="js_legstyle" value="legs" />
<input type="hidden" name="js_legqty" id="js_legqty" value="<%=totalLegQty%>" />

<input type="hidden" name="js_valance" id="js_valance" value="Valance" />
<input type="hidden" name="js_valance1width" id="js_valance1width" value="<%=valancewidth%>" />
<input type="hidden" name="js_valance1length" id="js_valance1length" value="<%=valancelength%>" />
<!-- end hidden fields -->
...
<%end if%>
	...
	tickingSelected();
	disableMattressExWorksDate();
	disableBaseExWorksDate();
	disableTopperExWorksDate();
	disableHeadboardExWorksDate();
	disableLegsExWorksDate();
	disableValanceExWorksDate();
	$('#minus1').hide();
	$('#plus1').show();
	checkWrapShow();
	checkCrateShow();
...
<% if rs("mattressrequired")="y" then %>
	...
	if (wraptype == 4) {
		// crate
		defaultCrateSize('1','matt1_packwidth','matt1_packheight','matt1_packdepth','mattressCrateType','matt_crateqty', true);
		defaultCrateSize('1','matt2_packwidth','matt2_packheight','matt2_packdepth','mattressCrateType2','matt_crateqty', true);
		defaultCrateWeight('1','mattressCrateType','matt1cratewgt','matt_crateqty', true);
		defaultCrateWeight('1','mattressCrateType2','matt2cratewgt','matt_crateqty', true);
		defaultCompWeight('1', 'js_savoirmodel', 'js_matt1width', 'js_matt1length', 'crateprodwgt1_1', true);
		defaultCompWeight('1', 'js_savoirmodel', 'js_matt2width', 'js_matt2length', 'crateprodwgt1_2', true);
		setTotalWeight('matt1cratewgt', 'crateprodwgt1_1', 'matt1_packkg');
		setTotalWeight('matt2cratewgt', 'crateprodwgt1_2', 'matt2_packkg');
	}
	if (wraptype == 3) {
		// box
		defaultBoxSize('1','matt1boxsize','matt2boxsize',true);
		defaultBoxWeight('1', 'matt1boxsize', 'matt1boxwgt', true);
		defaultBoxWeight('1', 'matt2boxsize', 'matt2boxwgt', true);
		defaultCompWeight('1', 'js_savoirmodel', 'js_matt1width', 'js_matt1length', 'boxprodwgt1_1', true);
		defaultCompWeight('1', 'js_savoirmodel', 'js_matt2width', 'js_matt2length', 'boxprodwgt1_2', true);
...
<% if rs("baserequired")="y" then %>
	if (wraptype == 4) {
		defaultCrateSize('3','base1_packwidth','base1_packheight','base1_packdepth','baseCrateType','base_crateqty', true);
		defaultCrateSize('3','base2_packwidth','base2_packheight','base2_packdepth','baseCrateType2','base2_crateqty', true);
		defaultCrateWeight('3','baseCrateType','base1cratewgt','base_crateqty', true);
		defaultCrateWeight('3','baseCrateType','base2cratewgt','base_crateqty', true);
		defaultCompWeight('3', 'js_basesavoirmodel', 'js_base1width', 'js_base1length', 'crateprodwgt3_1', true);
		defaultCompWeight('3', 'js_basesavoirmodel', 'js_base2width', 'js_base2length', 'crateprodwgt3_2', true);
		setTotalWeight('base1cratewgt', 'crateprodwgt3_1', 'base1_packkg');
		setTotalWeight('base2cratewgt', 'crateprodwgt3_2', 'base2_packkg');
	}
	if (wraptype == 3) {
		// box
		defaultBoxSize('3','base1boxsize','base2boxsize',true);
		defaultBoxWeight('3','base1boxsize','base1boxwgt', true);
		defaultBoxWeight('3','base2boxsize','base2boxwgt', true);
		defaultCompWeight('3', 'js_basesavoirmodel', 'js_base1width', 'js_base1length', 'boxprodwgt3_1', true);
		defaultCompWeight('3', 'js_basesavoirmodel', 'js_base2width', 'js_base2length', 'boxprodwgt3_2', true);
...
<% if rs("topperrequired")="y" then %>
	if (wraptype == 4) {
		defaultCrateSize('5','topper1_packwidth','topper1_packheight','topper1_packdepth','topperCrateType','topper_crateqty', true);
		defaultCrateWeight('5','topperCrateType','topper1cratewgt','topper_crateqty', true);
		defaultCompWeight('5', 'js_toppertype', 'js_topper1width', 'js_topper1length', 'crateprodwgt5_1', true);
		setTotalWeight('topper1cratewgt', 'crateprodwgt5_1', 'topper1_packkg');
	}
	if (wraptype == 3) {
		// box
		defaultBoxSize('5','topper1boxsize','',true);
		defaultBoxWeight('5','topper1boxsize','topper1boxwgt', true);
		defaultCompWeight('5', 'js_toppertype', 'js_topper1width', 'js_topper1length', 'boxprodwgt5_1', true);
...
<% if rs("valancerequired")="y" then %>
	if (wraptype == 4) {
		defaultCrateSize('6','valance1_packwidth','valance1_packheight','valance1_packdepth','valanceCrateType','valance_crateqty', true); 
		defaultCrateWeight('6','valanceCrateType','valance1cratewgt','valance_crateqty', true);
		defaultCompWeight('6', 'js_valance', 'js_valance1width', 'js_valance1length', 'crateprodwgt6_1', true);
		setTotalWeight('valance1cratewgt', 'crateprodwgt6_1', 'valance1_packkg');
	}
	if (wraptype == 3) {
		// box
		defaultBoxSize('6','valance1boxsize','', true);
		defaultBoxWeight('6','valance1boxsize','valance1boxwgt', true);
		defaultCompWeight('6', 'js_valance', 'js_valance1width', 'js_valance1length', 'boxprodwgt6_1', true);
		setTotalWeight('valance1boxwgt', 'boxprodwgt6_1', 'valance1kg');
	}
<% end if %>
<% if rs("legsrequired")="y" then %>

	defaultCrateSize('7','legs1_packwidth','legs1_packheight','legs1_packdepth','legsCrateType','legs_crateqty', true);
	defaultCrateWeight('7','legsCrateType','legs1cratewgt','legs_crateqty', true);
	defaultCompWeight('7', 'js_legstyle', 'legs1boxsize', '', 'crateprodwgt7_1', true);
	setTotalWeight('legs1cratewgt', 'crateprodwgt7_1', 'legs1_packkg');
	
	defaultBoxSize('7','legs1boxsize','',true);
	defaultBoxWeight('7','legs1boxsize','legs1boxwgt', true);
	defaultCompWeight('7', 'js_legstyle', 'js_legqty', '', 'boxprodwgt7_1', true);
...
<% if rs("headboardrequired")="y" then %>
	...
	if (wraptype == 3) {
		defaultBoxSize('8','hb1boxsize','hb2boxsize',true);
		defaultBoxWeight('8', 'hb1boxsize', 'hb1boxwgt', true);
		defaultBoxWeight('8', 'hb2boxsize', 'hb2boxwgt', true);
		defaultCompWeight('8', 'js_headboardstyle', 'js_hb1width', '', 'boxprodwgt8_1', true);
		defaultCompWeight('8', 'js_headboardstyle', 'js_hb1width', '', 'boxprodwgt8_2', true);
		setTotalWeight('hb1boxwgt', 'boxprodwgt8_1', 'hb1kg');
		setTotalWeight('hb2boxwgt', 'boxprodwgt8_2', 'hb2kg');
	}
	if (wraptype == 1 || wraptype == 2) {
		defaultCompWeight('8', 'js_headboardstyle', 'js_hb1width', '', 'hb_packkg', true);
...
defaultBoxSize('9','acc1boxsize','', true);
<% end if %>
...
set rs3=nothing%>
	...
	showHideExWorksAccessories('exworksacc_', 'accdelmethod', 'exworksdateacc_');
...
function setVIPAlert() {
	{
            ...
            if (checkBox.checked) {
                // If checked, show the alert
                alert("VIP MEMBER\nPlease note, this customer is part of the VIP Member club and as such the alternate VIP gift box should be provided with the delivery. Please ensure the VIP gift box is correctly provided and delivered to this customer.");
            }
...
function disableMattressExWorksDate() {
	var mattstatus=$('#mattressqc').val();
	if (mattstatus > 4) {
		$('#exworksdatematt option:not(:selected)').attr('disabled', true);
	} else {
		$('#exworksdatematt option:not(:selected)').attr('disabled', false);
	}
}
function disableBaseExWorksDate() {
	var basestatus=$('#baseqc').val();
	if (basestatus > 4) {
		$('#exworksdatebase option:not(:selected)').attr('disabled', true);
	} else {
		$('#exworksdatebase option:not(:selected)').attr('disabled', false);
	}
}
function disableTopperExWorksDate() {
	var topperstatus=$('#topperstatus').val();
	if (topperstatus > 4) {
		$('#exworksdatetopper option:not(:selected)').attr('disabled', true);
	} else {
		$('#exworksdatetopper option:not(:selected)').attr('disabled', false);
	}
}
function disableHeadboardExWorksDate() {
	var hbstatus=$('#headboardstatus').val();
	if (hbstatus > 4) {
		$('#exworksdatehb option:not(:selected)').attr('disabled', true);
	} else {
		$('#exworksdatehb option:not(:selected)').attr('disabled', false);
	}
}
function disableLegsExWorksDate() {
	var legstatus=$('#legsstatus').val();
	if (legstatus > 4) {
		$('#exworksdatelegs option:not(:selected)').attr('disabled', true);
	} else {
		$('#exworksdatelegs option:not(:selected)').attr('disabled', false);
	}
}
function disableValanceExWorksDate() {
	var valancestatus=$('#valancestatus').val();
	if (valancestatus > 4) {
		$('#exworksdatevalance option:not(:selected)').attr('disabled', true);
	} else {
		$('#exworksdatevalance option:not(:selected)').attr('disabled', false);
	}
}
...
function showHideExWorksAccessories(exWorksDivId, delMthdCtrlId, exWorksDateCtrlId) {
	for (let i = 1; i < 21; i++) {
		let show = showHideExWorks(exWorksDivId+i, delMthdCtrlId, exWorksDateCtrlId+1);
		if (typeof show !== 'undefined') {
			if (show) {
				$('.acc_exworksdate').show();
			} else {
				$('.acc_exworksdate').hide();
			}
		}
	}
}
...
<%
function makeAccessoriesExworksDateDropdown(byref acon, aCompItemNo)
	' run in live just once - update exportlinks set compItemNo=1 where compItemNo is null and componentID=9
	dim ars5, ars6, aHtml
	if retrieveUserLocation()=1  or retrieveUserLocation()=27 or userHasRoleInList("ORDERDETAILS_VIEWER") then
		Set ars5 = getMysqlUpdateRecordSet("Select * from exportcollections E, exportCollShowrooms C, Location L where E.collectionStatus=1 and E.exportCollectionsID=C.exportcollectionID and C.idlocation=L.idlocation", acon)
	else
		Set ars5 = getMysqlUpdateRecordSet("Select * from exportcollections E, exportCollShowrooms C, Location L where  E.exportCollectionsID=C.exportcollectionID and C.idlocation=L.idlocation and C.idlocation=" & retrieveUserLocation(), acon)
	end if
	aHtml = "<select id='exworksdateacc_" & aCompItemNo & "' name='exworksdateacc_" & aCompItemNo & "'>"
	currentexworksdate2=""
	currentexworksdate=""
	Set ars6 = getMysqlQueryRecordSet("Select * from exportlinks L, exportCollShowrooms S, exportcollections E where L.purchase_no=" & rs("purchase_no") & " and L.linksCollectionID=S.exportCollshowroomsID and E.exportCollectionsID=S.exportCollectionID AND L.componentID=9 and L.compItemNo=" & aCompItemNo, acon)
	if not ars6.eof then
		currentexworksdate2 = ars6("CollectionDate")
		currentexworksdate = ars6("linkscollectionid")
	end if
	ars6.close
	set ars6=nothing
	if currentexworksdate = "" then
		aHtml = aHtml & "<option value='n' selected>TBA</option>"
	else
		aHtml = aHtml & "<option value='" & currentexworksdate & "' selected><" & currentexworksdate2 & "></option>"
	end if
	Do until ars5.eof
		optionselected = "" 
		if currentexworksdate<>"" then
			if CInt(currentexworksdate)=ars5("exportCollshowroomsID")  then optionselected = "selected"
		end if
        aHtml = aHtml & "<option value='" & ars5("exportCollshowroomsID") & "' " & optionselected & ">" & ars5("adminheading") & ", " & ars5("CollectionDate") & "</option>"
        ars5.movenext
	loop
	ars5.close
	set ars5=nothing
	aHtml = aHtml & "</select>"
	makeAccessoriesExworksDateDropdown = aHtml
end function

function getOrderType(byref acon, byref ars)
	dim ars2
	if isnull(ars("ordertype")) or ars("ordertype") = "" then
		getOrderType = "Unknown"
	else
		Set ars2 = getMysqlUpdateRecordSet("Select * from ORDERTYPE where ordertypeid=" & ars("ordertype"), acon)
