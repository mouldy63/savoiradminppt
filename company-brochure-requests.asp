...
<%
dim ALLOWED_ROLES
ALLOWED_ROLES = "ADMINISTRATOR,TRADE"
%>
...
<%Dim postcode, postcodefull, Con, rs, rs1, sql, recordfound, id, rspostcode, submit, count, msg, fontclass
msg=""
msg=Request("msg")
count=0
...
if not isSuperuser() then
	...
	If retrieveUserLocation()<>3 and retrieveuserregion()<>9 then
		sql = sql & " AND C.idlocation in (" & makeBuddyLocationList(retrieveUserLocation(), con) & ")"
	end if
...
Set Con = Nothing%>
   ...
   <%end if%>
    ...
    <input type="submit" name="submit4" id="submit4" value="Delete Entries (spam)" onClick="return confirm('Are you sure you want to DELETE these brochure requests?'); ">
