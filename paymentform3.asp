...
<%'if1
if idlocation<>14 then
	if pendinginvoiceexists="y" then response.Write("<p>Invoice number " & pendinginvoiceNo & " and invoice date " & pendinginvoiceDate & " has been allocated to the deposit <a href=""savoir-invoice-deposit.asp?invdt=" & pendinginvoiceDate & "&invno=" & pendinginvoiceNo & "&pno=" & order & """ target=""_blank""><font color=""red""><b>>> PRINT DEPOSIT <<</b></font></a></p><br />")
end if
if orderhasexports="y" then%>
 ...
 <option value="savoir-parts-invoice.asp?invno=YYY&cid=XXX&pno=<%=order%>">Standard Parts Invoice</option>
 </select></td>
 <%'if2
...
if invoicenumbersexistfororder="y" then%>
  ...
  <table border="0" cellpadding="2" class="rowlineheight">
  <tr>
