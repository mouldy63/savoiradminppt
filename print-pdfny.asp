...
<%Dim postcode, postcodefull, Con, rs, recordfound, id, rspostcode, submit, count, correspondence, found, item, msg2, ItemValue, e1, orderno, mattressrequired, mattressprice, topperrequired, topperprice, baserequired, baseprice, upholsteredbase, upholsteryprice, valancerequired, accessoriesrequired, valanceprice, bedsettotal, headboardrequired, headboardprice, deliverycharge, deliveryprice, total, val, contact,  orderdate, reference, clientstitle, clientsfirst, clientssurname, deldate, add1, add2, add3, town, county, country, add1d, add2d, add3d, townd, countyd, postcoded, countryd, deliveryinstructions, savoirmodel, mattresstype, tickingoptions, mattresswidth, mattresslength, leftsupport, rightsupport, ventposition, ventfinish, mattressinstructions, toppertype, topperwidth, topperlength, toppertickingoptions, specialinstructionstopper, basesavoirmodel, basetype, basestyle, basewidth, baselength, legstyle, legfinish, legheight, linkposition, linkfinish, baseinstructions, basefabric, basefabricchoice, headboardstyle, headboardfabric, headboardfabricchoice, headboardheight, specialinstructionslegs, specialinstructionsheadboard, pleats, valancefabric, valancefabricchoice, specialinstructionsvalance, specialinstructionsdelivery, sql, localeref, order, rs1, rs2, rs3, selcted, custcode, msg, signature, custname, quote, showroomaddress, custaddress, s, deliveryaddress, clientdetails, clienthdg, str2, str3, str4, str5, str6, str7, str8, valreq, valancetotal, sumtbl, basevalanceprice, discountamt, termstext, xacc, accesscost, accessoriesonly, ademail, ademail2, aw, x, str18, str118, baseupholsteryprice, legprice, basefabricdesc, headboardfabricdesc
Dim matt1width, matt2width, matt1length, matt2length, base1width, base2width, base1length, base2length, topper1width, topper1length, basewidthstring, mattwidthstring, topperwidthstring, speciallegheight, savoirterms, legtotal, NPage, P, OrderTotalExVAT, VATWording, OrderTotalIncVAT, nyx
if retrieveuserlocation()=34 then
OrderTotalExVAT="Sub Total"
VATWording="NY Tax"
OrderTotalIncVAT="Order Total"
else
OrderTotalExVAT="Order Total, Ex VAT"
VATWording="VAT"
OrderTotalIncVAT="Order Total inc. VAT"
end if
...
Set rs = getMysqlUpdateRecordSet("Select * from purchase P, savoir_user S where P.purchase_no=" & purchase_no & " and P.salesusername like S.username", con)
if not rs.eof then
...
If ademail2<>"" then showroomaddress=showroomaddress & "&nbsp;&nbsp;Email: " &ademail2
rs2.close
set rs2=nothing
...
xacc="<table><tr><td width=""10"" height=""20""></td><td>Item&nbsp;Description</td><td>Design</td><td>Colour</td><td>Size</td><td align=""right"">Qty</td>"
...
If rs("istrade")="y" then
	...
	If rs("tradediscount")<>"" and NOT ISNULL(rs("tradediscount")) then
		If  CDbl(rs("tradediscount"))>0.0 then sumtbl=sumtbl & "<tr><td width=10></td><td width=" & nytablewidth & ">Trade Discount</td>"
		if NOT userHasRoleInList("NOPRICESUSER") then
'if (retrieveuserid()<>181 and retrieveuserid()<>182) then
	...
	sumtbl=sumtbl & "<tr><td width=10></td><td width=" & nytablewidth & "><b>" & OrderTotalExVAT & "</b></td>"
		if NOT userHasRoleInList("NOPRICESUSER") then
'if (retrieveuserid()<>181 and retrieveuserid()<>182) then
			sumtbl=sumtbl & "<td width=25 align=""right""><b>" & fmtCurr2(rs("subtotal"), true, rs("ordercurrency")) & "</b></td>"
		end if
	sumtbl=sumtbl & "</tr>"
	If rs("vat")<>"" and rs("vat")<>"0" and NOT ISNULL(rs("vat")) then
		If  CDbl(rs("vat"))>0.0 then sumtbl=sumtbl & "<tr><td width=10></td><td width=" & nytablewidth & ">" & VATWording & "</td>"
		if NOT userHasRoleInList("NOPRICESUSER") then
'if (retrieveuserid()<>181 and retrieveuserid()<>182) then
...
If rs("totalexvat")<>"" and NOT ISNULL(rs("totalexvat")) then
	If  CDbl(rs("totalexvat"))>0.0 then sumtbl=sumtbl & "<tr><td width=10></td><td width=" & nytablewidth & ">Total Ex. VAT</td>"
	if NOT userHasRoleInList("NOPRICESUSER") then
'if (retrieveuserid()<>181 and retrieveuserid()<>182) then
...
If rs("vat")<>"" and NOT ISNULL(rs("vat")) then
	If CDbl(rs("vat"))<>0 then
		If  CDbl(rs("vat"))>0.0 then sumtbl=sumtbl & "<tr><td width=10></td><td width=" & nytablewidth & ">VAT</td>"
		if NOT userHasRoleInList("NOPRICESUSER") then
	'if (retrieveuserid()<>181 and retrieveuserid()<>182) then
			sumtbl=sumtbl & "<td width=25 align=""right""><b>" & fmtCurr2(rs("vat"), true, rs("ordercurrency")) & "</b></td>"
		end if
...
sumtbl=sumtbl & "<tr><td width=10></td><td width=" & nytablewidth & "><b>Order Total inc. VAT</b></td>"
	if NOT userHasRoleInList("NOPRICESUSER") then
'if (retrieveuserid()<>181 and retrieveuserid()<>182) then
		sumtbl=sumtbl & "<td width=25 align=""right""><b>" & fmtCurr2(rs("total"), true, rs("ordercurrency")) & "</b></td>"
	end if
...
sumtbl=sumtbl & "<tr><td width=10></td><td width=215>Accessories (see next page for details)</td>"
	if NOT userHasRoleInList("NOPRICESUSER") then
'if (retrieveuserid()<>181 and retrieveuserid()<>182) then
		sumtbl=sumtbl & "<td width=25 align=""right""><b>" & fmtCurr2(rs("accessoriestotalcost"), true, rs("ordercurrency")) & "</b></td>"
	end if
...
If rs("discount")<>"" and NOT ISNULL(rs("discount")) then
	If CDbl(rs("discount"))>0.0 then
		If rs("discounttype")="percent" then
		 discountamt=CDbl(rs("bedsettotal"))*(CDbl(rs("discount"))/100)
		sumtbl=sumtbl & "<tr><td width=10></td><td width=215>Discount</td>"
			if NOT userHasRoleInList("NOPRICESUSER") then
'if (retrieveuserid()<>181 and retrieveuserid()<>182) then
				sumtbl=sumtbl & "<td width=25 align=""right"">" & fmtCurr2(discountamt, true, rs("ordercurrency")) & "</td>"
			end if
			sumtbl=sumtbl & "</tr>"	
		end if
	end if
...
If rs("istrade")="y" then
	...
	If rs("tradediscount")<>"" and NOT ISNULL(rs("tradediscount")) then
		If  CDbl(rs("tradediscount"))>0.0 then sumtbl=sumtbl & "<tr><td width=10></td><td width=215>Trade Discount</td>"
			if NOT userHasRoleInList("NOPRICESUSER") then
'if (retrieveuserid()<>181 and retrieveuserid()<>182) then
				sumtbl=sumtbl & "<td width=25 align=""right"">" & fmtCurr2(rs("tradediscount"), true, rs("ordercurrency")) & "</td>"
			end if
			sumtbl=sumtbl & "</tr>"
	end if
	
	sumtbl=sumtbl & "<tr><td width=10></td><td width=215><b>" & OrderTotalExVAT & "</b></td>"
		if NOT userHasRoleInList("NOPRICESUSER") then
'if (retrieveuserid()<>181 and retrieveuserid()<>182) then
			sumtbl=sumtbl & "<td width=25 align=""right"">" & fmtCurr2(rs("subtotal"), true, rs("ordercurrency")) & "</td>"
		end if
		sumtbl=sumtbl & "</tr>"
	If rs("vat")<>"" and NOT ISNULL(rs("vat")) then
		If  CDbl(rs("vat"))>0.0 then sumtbl=sumtbl & "<tr><td width=10></td><td width=215>" & VATWording & "</td>"
			if NOT userHasRoleInList("NOPRICESUSER") then
'if (retrieveuserid()<>181 and retrieveuserid()<>182) then
				sumtbl=sumtbl & "<td width=25 align=""right"">" & fmtCurr2(rs("vat"), true, rs("ordercurrency")) & "</td>"
			end if	
			sumtbl=sumtbl & "</tr>"
	end if 
	sumtbl=sumtbl & "<tr><td width=10></td><td width=215><b>" & OrderTotalIncVAT & "</b></td>"
		if NOT userHasRoleInList("NOPRICESUSER") then
'if (retrieveuserid()<>181 and retrieveuserid()<>182) then
			sumtbl=sumtbl & "<td width=25 align=""right""><b>" & fmtCurr2(rs("total"), true, rs("ordercurrency")) & "</b></td>"
		end if	
...
If rs("totalexvat")<>"" and NOT ISNULL(rs("totalexvat")) then
	If  CDbl(rs("totalexvat"))<>"0.00" then sumtbl=sumtbl & "<tr><td width=10></td><td width=215>Total Ex. VAT</td>"
		if NOT userHasRoleInList("NOPRICESUSER") then
'if (retrieveuserid()<>181 and retrieveuserid()<>182) then
			sumtbl=sumtbl & "<td width=25 align=""right"">" & fmtCurr2(rs("totalexvat"), true, rs("ordercurrency")) & "</td>"	
		end if	
		sumtbl=sumtbl & "</tr>"
end if 
...
If rs("vat")<>"" and NOT ISNULL(rs("vat")) then
	if CDbl(rs("vat"))<>0 then
		If  CDbl(rs("vat"))>0.0 then sumtbl=sumtbl & "<tr><td width=10></td><td width=215>VAT</td>"
			if NOT userHasRoleInList("NOPRICESUSER") then
'if (retrieveuserid()<>181 and retrieveuserid()<>182) then
			sumtbl=sumtbl & "<td width=25 align=""right"">" & fmtCurr2(rs("vat"), true, rs("ordercurrency")) & "u</td>"
			
			end if	
		sumtbl=sumtbl & "</tr>"	
	end if
end if 
sumtbl=sumtbl & "<tr><td width=10></td><td width=215><b>Order Total inc. VAT</b></td>"
	if NOT userHasRoleInList("NOPRICESUSER") then
'if (retrieveuserid()<>181 and retrieveuserid()<>182) then
		sumtbl=sumtbl & "<td width=25 align=""right""><b>" & fmtCurr2(rs("total"), true, rs("ordercurrency")) & "</b></td>"
	end if	
