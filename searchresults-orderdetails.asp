...
<%
Dim con, rs, contactNo, current, sql, dimensions, orderdate, cref, filterType, filterVal
dimensions="n"
...
sql = "select p.purchase_no,p.order_number,p.order_date,p.customerreference,p.bed,p.notes"
sql = sql & " from purchase p "
sql = sql & " where (P.cancelled is null or P.cancelled = 'n') and p.contact_no=" & contactNo
...
if cref <> "" then
	sql = sql & " and p.customerreference like '%" & cref & "%'"
end if
if (filterType = "company" or filterType = "custref" or filterType = "A") and filterVal <> "" then
	if filterType = "company" then
		sql = sql & " and p.companyname like '%" & filterVal & "%'"
	elseif filterType = "custref" then
		sql = sql & " and p.customerreference like '%" & filterVal & "%'"
	elseif filterType = "A" then
		sql = sql & " and (p.companyname like '%" & filterVal & "%'"
		sql = sql & " or p.customerreference like '%" & filterVal & "%')"
	end if
end if
