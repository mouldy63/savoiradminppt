...
rs2("date")=date()
rs2("type")="Letter"
rs2("notes")="Brochure Request Follow-Up"
rs2("next")=DateAdd("d",14,date())
rs2("commstatus")="To Do"
...
'add additional brochure request follow up note
...
If rs("town")<>"" Then response.write(rs("town") & "<br />")
...
response.Write("<p>" & rs2("greeting") & " ")
	if lcase(left(rs("title"),3))="sir" then
	response.write(capitalise(rs("title")) & " ")
	else
	If rs("title")<>"" Then response.write(capitalise(rs("title")) & " ")
