<!--'Code beautified 14/10/2014 16:19:00 by ASP-Beautify V1.5  © 1999-2000 by Dirk Paessler & Zack Jones - http://www.web-server-tools-->
<div id="payment_table_standalone">
<form method="post" name="form_payments_standalone" id="form_payments_standalone" action="updatepayments1.asp" onSubmit="return validatePaymentsForm(this);">
<% orderhasexports="y" %>
<%if orderhasexports="y" then%>

 <table border="5" align="right" class="delfloatright" bgcolor="#CCCCCC">
 <tr>
   <%if userHasRoleInList("DEPOSITFINALINVOICE") then%>
 <td valign="top" class="floatleft"><strong>Invoice Type</strong> <br>
 ...
 <option value="commercial-invoice.asp?cid=XXX&pno=<%=order%>">Commercial Invoice</option>
 <option value="savoir-invoice.asp?invno=YYY&cid=XXX&pno=<%=order%>">Standard Invoice</option>
  <%if userHasRoleInList("DEPOSITFINALINVOICE") then%>
 <option value="savoir-invoice-deposit.asp?invno=YYY&cid=XXX&pno=<%=order%>">Deposit Invoice</option>
 <option value="savoir-invoice-final.asp?invno=YYY&cid=XXX&pno=<%=order%>">Final Invoice</option>
  <%end if%>
 </select></td>
...
if invoicenumbersexistfororder="y" then%>
  ...
  <table border="1" cellpadding="2" class="rowlineheight">
  <tr>
