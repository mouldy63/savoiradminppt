...
<%Dim postcode, postcodefull, Con, rs, recordfound, id, rspostcode, submit, count, sql, msg, dateasc, orderasc, customerasc, coasc, datefrom, datefromstr, datetostr, dateto, dateto1, user, rs2, rs3, receiptasc, amttotal, currencysym, ordervaltotal, totalpayments, totalos, location
location=""
...
<form name="form1" method="post" action="accounts-csv.asp">
...
<!--<form name="form1" method="post" action="">-->	
  <p>
    <%
sql = "Select P.purchase_no, P.order_number, Z.amount, P.total, P.Paymentstotal, P.idlocation, Z.placed, Z.salesusername, Z.receiptno, A.company, P.balanceoutstanding, C.surname, C.title, C.first, Z.paymenttype, P.order_date, Z.paymentid, Z.invoice_number, P.customerreference, Z.invoicedate from address A, contact C, Purchase P,  Payment Z Where C.retire='n' AND A.code=C.code AND C.Code<>218766 AND C.code<>213190 AND C.code=P.code AND P.purchase_no=Z.purchase_no AND P.quote='n' and P.salesusername<>'dave' AND P.salesusername<>'maddy' "
...
<a href="accounts.asp?location=<%=location%>&customerasc=a&datefrom=<%=datefrom%>&dateto=<%=datetostr%>&user=<%=request("user")%>"><img src="img/asc.gif" alt="Ascending" width="24" height="20" align="middle"border></a></b></td>
    <td valign="bottom">Customer Ref.</td>
...
<a href="accounts.asp?location=<%=location%>&orderasc=a&datefrom=<%=datefrom%>&dateto=<%=datetostr%>&user=<%=request("user")%>"><img src="img/asc.gif" alt="Ascending" width="24" height="20" align="middle" border="0"></a></b></td>
       <td align="right" valign="bottom">
        Payment Amount<b></b></td>
       <td valign="bottom">Payment Type</td>
       ...
       <td valign="bottom">Invoice No.</td>
       <td valign="bottom">Invoice Date:</td>
...
amttotal=amttotal+CCur(rs("amount")) 
...
totalos=totalos+CCur(rs("balanceoutstanding"))%>
  <tr>
  <td valign="bottom"><%=DateValue(rs("placed"))%></td>
...
%></td>
    <td valign="bottom"><%=rs("customerreference")%></td>
    <td valign="bottom"><%
If rs("company")<>"" then response.write(rs("company"))
response.Write("</a>")%></td>
    <td valign="bottom"><%response.Write(rs("order_number"))%>
      &nbsp;</td>
       <% if userHasRoleInList("ACCOUNTS") then %>
        <td align="right" valign="bottom">
        
		<%If rs("amount")<>"" then response.write("<a href=""amendpmt.asp?location=" & location & "&datefrom=" & datefrom & "&dateto=" & datetostr & "&user=" & request("user") & "&val=" & rs("paymentid") & """>" & fmtCurr2(abs(cdbl(rs("amount"))), true, rs3("currency")) & "</a>")
		 'If rs("dateofamend")<>"" then response.Write("<br />Amended on " & rs("dateofamend") & " by " & rs("amendedby") & " - " & rs("reasonforamend"))%></td>
        <%else%>
        <td align="right" valign="bottom"><%If rs("amount")<>"" then response.write(fmtCurr2(abs(cdbl(rs("amount"))), true, rs3("currency")))%></td>
        <%end if%>
        <td valign="bottom"><%If rs("paymenttype")<>"" then response.write(rs("paymenttype"))%></td>
        <td valign="bottom"><%If rs("receiptno")<>"" then response.write("R" & rs("receiptno"))%></td>
        ...
        <td valign="bottom"><%=rs("invoicedate")%>&nbsp;</td>
        <td valign="bottom"><%If rs("invoice_number")<>"" then response.write(rs("invoice_number"))%></td>
...
<td align="right" valign="bottom"><b><%=fmtCurr2(amttotal, true, currencysym)%></b></td>
<td colspan="6" valign="bottom">&nbsp;</td>
<td align="right" valign="bottom"><b><%=fmtCurr2(ordervaltotal, true, currencysym)%></b></td>
<td valign="bottom"><b><%=fmtCurr2(totalpayments, true, currencysym)%></b></td>
<td valign="bottom"><b><%=fmtCurr2(totalos, true, currencysym)%></b></td>
</tr>
</table>
