...
Set rs = getMysqlUpdateRecordSet(sql, con)
if not rs.eof then
arraycount=rs.recordcount-1
arraycounter=0
Dim componentarray(CInt(arraycount))
shipmentdate=rs("CollectionDate")
do until rs.eof
componentarray(arraycounter)=rs("componentid")
...
if accessoriesonly="n" then 
		...
		If rs("bookeddeliverydate")<>"" then
		PDF.AddHtmlPos 20, 20, "Delivery Date: <b>" & FormatDateTime(rs("bookeddeliverydate"),vbShortDate) & "</b>"
		else
		PDF.AddHtmlPos 20, 20, "Ex-Works Date: " & shipmentdate
		end if
		
		
		PDF.AddHtmlPos 20, 35, "Delivery Time:<b> " & deltime & "</b>"
		PDF.AddHtmlPos 20, 50, "Delivered by: <b>" & deliveredby & "</b>"
...
PDF.AddHtmlPos 20, 50, "Delivery Time: " & deltime
PDF.AddHtmlPos 20, 70, "Showroom: " & showroomaddress
