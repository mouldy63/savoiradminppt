...
<div class="row">
			 			  <label for="acceptemail"><strong>Customer accepts email marketing (ticked if yes)</strong></label>
...
<br>
			...
			<%end if %><input type="button" id="viptooltip" value="?" title=" VIP Member is automatically set to yes when the following criteria are met.<br><br>Cumulative spend of £ 19,999 inc VAT, either<br><br>Harrods, Mayfair or Chelsea as showroom, and customer type set to Private." /> 
</div>
		</div></div>
			    <div class="TabbedPanelsContent">
              <div class="two-col">
...
if isSuperuser or rs2("Status")<>"Customer" or rs1("Status") = "Customer" then %>
 ...
 <div class="row"><b>Last Contact Date:</b><br>
  ...
  <tr>
	...
	if rs2("cancelled")="y" then
		response.write("<a class=""greytext"" href=""/php/order?pn=" & rs2("purchase_no") & """>" & rs2("order_number") & " CANCELLED ORDER</a>")
	elseif rs2("completedorders")="n" AND rs2("orderonhold")="n" and rs2("quote")="d" then
		response.write("<a class=""greentext"" href=""/php/order?quote=y&pn=" & rs2("purchase_no") & """>" & rs2("order_number") & " Declined</a>")
	elseif rs2("completedorders")="n" AND rs2("orderonhold")="n" and rs2("quote")="y" then
		response.write("<a class=""greentext"" href=""/php/order?quote=y&pn=" & rs2("purchase_no") & """>" & rs2("order_number") & "</a>")
	elseif rs2("completedorders")="n" AND rs2("orderonhold")="n" and rs2("quote")="n" then
		response.write("<a class=""redtext"" href=""/php/order?pn=" & rs2("purchase_no") & """>" & rs2("order_number") & "</a>")
	elseif rs2("orderonhold")="y" AND rs2("completedorders")="n" then
		response.write("<a class=""bluetext"" href=""/php/order?pn=" & rs2("purchase_no") & """>" & rs2("order_number") & "</a>")
	elseif rs2("completedorders")="y" then 
		response.write("<a href=""/php/order?readonly=y&pn=" & rs2("purchase_no") & """>" & rs2("order_number") & "</a>")
	else
		' just in case
		response.write("<a href=""/php/order?readonly=y&pn=" & rs2("purchase_no") & """>" & rs2("order_number") & "</a>")
	end if%>&nbsp;</td>
...
set rs3=nothing%>
  ...
  <tr>
	...
	<%else%>
	<%=rs2("notes")%>&nbsp;
    <%if rs2("notes") = "Brochure Request Follow-Up" then
		if rs2("date")<>"" then
		response.Write("<br /><br /><b>Brochure Request Date:</b> " & rs2("date"))
		else
		response.Write("<br /><br /><b>Brochure Request Date:</b> " & rs1("first_contact_date"))
		end if
	end if%>
    <%end if%>
    </td>
