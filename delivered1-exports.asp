...
<%dim orderexists, Con, rs, sql, rs1, totalpn, items, n, itemtotal
itemtotal=0
dim pnarray(), count
count = 0
...
sql="SELECT * from exportcollections E, shipper_address S, collectionStatus C where E.shipper=S.shipper_address_id AND C.collectionStatusID=E.collectionStatus AND E.collectionStatus=4 order by CollectionDate"
else
	...
	Set rs1 = getMysqlQueryRecordSet(sql, con) 
		if not rs1.eof then
          ...
          <td valign="top"><%=rs("collectionStatusName")%></td>
           <%if retrieveUserLocation()=1 or retrieveUserLocation()=27  then%>
          <td valign="top"><a href="edit-collection.asp?collectionid=<%=rs("exportCollectionsID")%>">Edit</a>&nbsp;/&nbsp;Remove</td>
          <%end if%>
        </tr>
