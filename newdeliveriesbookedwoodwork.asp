...
<!-- #include file="orderfuncsTest.asp" -->
...
sql="Select WoodworkNoItems from manufacturedat where ManufacturedAtId=" & madeat
Set rs = getMysqlQueryRecordSet(sql, con)
WoodworkNoItems=rs("WoodworkNoItems")
rs.close
set rs=nothing
...
<tr><td height="35" colspan="10" bgcolor="#999999">&nbsp;<br>  <strong>Production Date</strong>
<div align="center" style="margin:0; padding:0; float:right; width:90%; font-size:14px;" >WOODWORK ROOM</div><br>&nbsp;</td></tr>
<tr><td colspan="10"><hr></td></tr>
<%if madeat=2 then
sql = "select * from address A, contact C, Purchase P, Location L Where P.code=A.code AND C.contact_no=P.contact_no AND P.completedorders='n' AND P.quote='n' AND P.completedorders='n' and P.orderonhold<>'y' and (P.cancelled is Null or P.cancelled='n') AND P.productiondate<>'' AND P.londonproductiondate<>'' and (P.DeliveryDateConfirmed<>'y' or P.DeliveryDateConfirmed is Null) AND P.source_site='SB' and P.idlocation=L.idlocation order by P.productiondate asc "
end if 
if madeat=1 then
sql = "select * from address A, contact C, Purchase P, Location L Where P.code=A.code AND C.contact_no=P.contact_no AND P.completedorders='n' AND P.quote='n' AND P.completedorders='n' and P.orderonhold<>'y' and (P.cancelled is Null or P.cancelled='n') AND P.productiondate<>'' AND P.cardiffproductiondate<>'' and (P.DeliveryDateConfirmed<>'y' or P.DeliveryDateConfirmed is Null) AND P.source_site='SB' and P.idlocation=L.idlocation order by P.productiondate asc "
end if 
...
finishedcomp3=isFrameFinished(con, rs("purchase_no"), 3)
compmadeat8=getComponentCurrentMadeAt(con, rs("purchase_no"), 8)
finishedcomp8=isFrameFinished(con, rs("purchase_no"), 8)
...
if (compmadeat3=1 and finishedcomp3="n") or (compmadeat8=1 and finishedcomp8="n") or (compmadeat7=1 and finishedcomp7="n") then
	hasCardiffItems = true
end if
if (compmadeat3=2 and finishedcomp3="n") or (compmadeat8=2 and finishedcomp8="n") or (compmadeat7=2 and finishedcomp7="n") then
	hasLondonItems = true
end if
ItemNotFinished=False
if finishedcomp3="n" or finishedcomp8="n" or finishedcomp7="n" then
...
productiondate=(DateAdd("d",-WoodworkNoItems,productiondate))
dofweek=Weekday(realproductiondate,0)
startweek=7-dofweek
pnewweek=DateAdd ("d", startweek, realproductiondate)

pweekno=DatePart("ww", productiondate)
weekNoToDisplay=DatePart("ww", productiondate)
...
response.Write("<tr><td height=""17"" colspan=""2"" bgcolor=""#999999"">Week No. " & weekNoToDisplay & "</td><td colspan=""6"" bgcolor=""#999999"" align=""center"">" & getcomponentWoodworkNoTobefinished(con, WoodworkNoItems, realProdStartDate, realProdEndDate, madeat) & " items to finish</td><td height=""17"" colspan=""2"" bgcolor=""#999999"">Production&nbsp;Date </td></tr>")
...
%>
    ...
    <td width="18%" valign="top"id="bespoke2">&nbsp;</td>
     ...
     <span style="white-space: pre;color:<%=getColourForEntry(con, rs("purchase_no"), "Cut", 3)%>">&#9608;</span>
      ...
      <td width="1%" valign="top">&nbsp;</td>
      <%end if%>
      <%if rs("legstyle")<>"" and NOT ISNULL(rs("legstyle")) and compmadeat7=madeat and finishedcomp7="n" then
	  %>
