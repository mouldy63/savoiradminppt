...
dim sizes, valancesize, hblegs, cid, showroomadd, invoicetotal, accprice, showroomnotes, invoicenotecount, paymentterms, bankdetails, paymentduedate, invdate, valfabricprice, valfabriccost
Dim legqty
invoicenotecount=1
invoicetotal=0
cid=request("cid")
...
sql="SELECT * from exportLinks L, exportcollshowrooms S where L.LinksCollectionID=S.exportCollshowroomsID and S.exportCollectionID=" & cid & " and purchase_no=" & purchase_no
Dim loc, expcolid
		'response.Write("sql=" & sql)
		Set rs1 = getMysqlQueryRecordSet(sql, con)
		loc=rs1("idlocation")
		expcolid=rs1("linkscollectionid")
		rs1.close
		set rs1=nothing

sql="SELECT * from exportLinks L where invoiceno='" & invno & "' and linksCollectionId='" & expcolid & "' and  purchase_no=" & purchase_no
