...
<%If submit<>"" Then 
%>
<p>Your blog has been amended on the website.</p>
	
<%sql="Select * from blogs WHERE blogid=" & val & ""
...
Else
Set rs = getMysqlQueryRecordSet("Select * from blogs WHERE blogid=" & val & "", con)
%>		<form action="editblog.asp" method="post" name="form1" onSubmit="return FrontPage_Form1_Validator(this)">
  <p>
  <label>Date
    <br>
  ...
  <p>
    <label>Author<br>
      <input name="author" type="text" id="author" value="<%=rs("author")%>" maxlength="255">
    </label>
  </p>
  <p>
...
<input name="title" type="text" id="title" value="<%=rs("title")%>" size="60" maxlength="255">
  ...
  <p><input name="val" type="hidden" id="val" value="<%=val%>">
    ...
    <input type="submit" name="submit" value="Amend Blog"  id="submit" class="button" />
  </p>
  <p>OR</p>
  <p>&nbsp;</p>
</form>
     <form method="post" action="deleteblog.asp" name="MyForm">
        <input type="hidden" name="val" value="<%=rs("blogID")%>">
        <input type="submit" value="Delete Blog" name="submit">
      </form>
<%
rs.close
set rs=nothing
